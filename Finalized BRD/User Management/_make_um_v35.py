# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v3.5.docx — vacant subordinate post switch only during login (no mid-session switch)."""
...
DST = BASE / "BRD_User_Management_v3.5.docx"
...
def build() -> Document:
    ...
    scope_in = {
        ...
        18: "OTP-based authentication (login, logout) — Username + OTP + Captcha; biometrics for departmental users; multi-post DSR login post selection; vacant subordinate post choice only during login (FR-53); no mid-session post switch",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "continue under the original login-selected post. Session roles and Module Function claims "
                "shall be derived from the post chosen at this login step via Post–Role mapping. After "
                "login is complete and the user has entered the application, the user shall not be "
                "allowed to switch post for the remainder of that session; a new login is required to "
                "choose a different post. Choice shall be audit-logged (actor, login post, chosen post, office).",
                "High",
            ),
        ],
    )
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Vacant subordinate switch", "Login-time only choice allowing a DSR Officer to act under a vacant child/descendant post of the login-selected post at the same office, cascading through vacant hierarchy nodes (FR-53); not available after login"),
    ...
    note2 = insert_paragraph_after(
        ...
        "they may choose a vacant subordinate post under the selected post (FR-53). After login "
        "completes, post cannot be switched until the next login.",
        "Normal",
    )
    ...
    login_post_tbl = insert_table_after(
        ...
        [
            ...
            ("7", "During login — optionally choose vacant subordinate post under login post", "DSR Officer / UI", "Hierarchy Master; same office; FR-53; before home"),
            ("8", "Lock session post; resolve Module Function claims; enter home", "UM / Auth", "No further post switch this session (FR-53)"),
        ],
    )

    switch_intro = insert_heading_after(
        login_post_tbl,
        "Vacant subordinate post choice during login only (DSR Officers — FR-53):",
        "Normal",
        parent=rbac_parent,
    )
    switch_note = insert_paragraph_after(
        ...
        "Office. Cascading: only vacant children are offered; under each vacant child, vacant "
        "grandchildren are also offered. Occupied posts are not offered and stop further descent. "
        "The officer may stay on the original login-selected post. Once the user confirms and enters "
        "the application, the session post is fixed — the user cannot switch post after login; a new "
        "login is required to choose differently.",
        "Normal",
    )
    switch_tbl = insert_table_after(
        ...
        [
            ...
            ("5", "Show options (Post Name + Office); include original login post", "UI", "Example: SR → FDA (vacant) → SDA (vacant)"),
            ("6", "User confirms one post for this session", "DSR Officer", "May keep login post or choose vacant subordinate"),
            ("7", "Lock session post; derive roles via Post–Role mapping; enter home", "UM / Auth", "FR-38, FR-47"),
            ("8", "After login — block any further post switch for this session", "System / UI", "Mandatory (FR-53); re-login to change"),
            ("9", "Audit log choice (login post, chosen post, office, actor, timestamp)", "System", "Mandatory"),
        ],
    )
    switch_ex_intro = insert_heading_after(
        switch_tbl,
        "Illustrative example (Office A — Sub-Registrar login; choice during login only):",
        "Normal",
        parent=rbac_parent,
    )
    switch_ex_tbl = insert_table_after(
        switch_ex_intro,
        ["Hierarchy (Office A)", "Vacancy at Office A", "Options during login for Sub-Registrar"],
        [
            ("Sub-Registrar (login-selected post)", "Occupied by self", "Always available (continue as Sub-Registrar)"),
            ("→ FDA (Enforcement) under Sub-Registrar", "Vacant", "May choose FDA (Enforcement) during login"),
            ("→ → SDA (Enforcement) under FDA", "Vacant", "May choose SDA (Enforcement) during login as well"),
            ...
            ("After home page entered", "—", "No post switch allowed until next login"),
        ],
    )
    ...
    enf_tbl = insert_table_after(
        ...
        [
            ...
            ("1b", "During login — optional vacant subordinate post at same Office (then lock session post)", "User / UI", "Hierarchy cascade while vacant (FR-53); no switch after home"),
    ...
    note7 = insert_paragraph_after(
        ...
        "subordinate post choice during login only (FR-53; no mid-session switch). Application Admin shall add, edit, reorder, "
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v3.5"
