...
| **Related inputs** | Requirement Discussions/Kaveri2.0/User Management KAVERI 2.0_v0.3.pdf; Requirement Discussions/BR_Discussion_Prep_Pack_User_Management_24Aug2026.docx; Requirement Discussions/ServiceDesk Issues/ServiceDeskIssuesList.xlsx; Project_Plan_Kaveri_3.0_Programme_v0.4 |
...
|---------|------|--------|-------------------|----------|
| 1.0 | 2026-08-26 | Nandha Kumar | Initial User Management BRD for Kaveri 3.0: FRS v0.3 baseline plus login, transfer, relieving, in-charge, DSC binding, audit and jurisdiction enforcement | Prashanth |
...
| ID | Title | Link |
|----|--------|------|
| BRD-K3-UM-001 | This document | Finalized BRD/User Management/BRD_User_Management_v1.0.docx |
| FRS-K2-UM-003 | Kaveri 2.0 User Management FRS v0.3 | Requirement Discussions/Kaveri2.0/User Management KAVERI 2.0_v0.3.pdf |
...
| ANNEX-UM-MAP | Role / group / module privilege matrix | To be rebuilt (UserMapping_v0.1_Annexure referenced by FRS is not packaged with the PDF) |
...
- 7.14 Status models
- 7.15 Process changes versus Kaveri 2.0 FRS
...
This document presents a comprehensive assessment of departmental **User Management** for Kaveri 3.0. It is based on the department’s stated target capability in the Kaveri 2.0 Functional Requirement Specification (User Management Module, v0.3), ServiceDesk evidence of live operational failures, and the Phase 1 programme mandate that User Management cover **login, roles, office mapping, transfer, relieving and RBAC**.

The current Kaveri 2.0 administration model already describes office hierarchy, posts, roles, groups, users and module-function mapping, but it does not reliably support credential lifecycle, in-charge / additional charge, relieving, DSC binding to the active post holder, or hard jurisdiction filters. These gaps surface as login failures, missing officer names in allocation dropdowns, digital-sign identity mismatches, and wrong-office / wrong-district visibility.
...
- Reduction in ServiceDesk “login / mapping / in-charge / digital sign identity” tickets versus the Kaveri 2.0 baseline (UM-PP-01 to UM-PP-10).
...
- Migration of agreed Kaveri 1.0 / 2.0 users, roles, office mappings and transfer / relieving history (depth per data-migration workstream).
...
|----|------------|-------------------|
| A-01 | Kaveri 2.0 FRS v0.3 remains the baseline capability to **keep and harden**, not to discard | PO, Domain Expert |
| A-02 | Official **email ID is the login name** unless DQ-04 decides otherwise (KGID / mobile / State SSO) | Security, PO |
| A-03 | **Group privilege overrides Role privilege** as stated in FRS, unless DQ-06 reverses this | PO, Security |
...
| A-06 | UserMapping_v0.1_Annexure will be retrieved or rebuilt as the signed privilege matrix before UAT | BA, AIGR Computers |
...
| A-10 | SMS gateway used on user create in Kaveri 2.0 remains available for 3.0 credential and alert messages | Ops, Arch |
...
| Create / change post | Non-mandatory in FRS v0.3; **Should** attach GO or equivalent order in 3.0 | Warning if missing; dual control for new post types |
...
|------|------------|--------|
| Super Admin (KPMU) | Statewide administrator responsible for creating users, roles, groups and granting privileges across the State | FRS v0.3 §1 |
| Department Admin | IGR acting as super-admin for administrative approvals and DIGR / AIGR assignment | FRS v0.3 §1 |
| Office Type | Classification of office: IGRO, DRO, SRO, Additional DRO, Additional SRO | FRS v0.3 §3.1 |
| Office | A concrete office instance (e.g. SRO Yeshwanthapura, DRO Mysuru) with jurisdiction, DDO (where applicable) and bilingual name | FRS v0.3 §3.2 |
| Jurisdiction ID | Parent office identifier that scopes data access (IGRO for DRO; DRO for SRO; self for IGRO) | FRS v0.3 §3.2 |
| DDO code | Drawing and Disbursing Officer code used with Khajane-II for that DRO / SRO | FRS v0.3 §3.2 |
| Short name | 3-character (rarely 4) office code used in the registration-number triplet | FRS v0.3 §3.2 |
| Anywhere Registration | SRO flag; if enabled for one SRO under a DRO, remaining SROs under the same DRO are also enabled (with user message) | FRS v0.3 §3.2 |
| Post | Occupiable position in an office, described as Role + Office Name (e.g. FDA SRO Chincholi) | FRS v0.3 §3.3 |
| Role | Named job function (IGR, DR, SR, FDA, SDA, DEO, …) to which module functions are mapped | FRS v0.3 §3.4 |
| Group | Set of users (e.g. IGR Office, DR Office, SR Office) that can carry privileges; **group privilege overrides role privilege** (FRS rule) | FRS v0.3 §3.9–3.11 |
| Function | Atomic privilege verb under a module / sub-module (view, print, scan, download, approve, sign, …) | FRS v0.3 §3.12 |
| Government Appointed | User type requiring KGID, date of joining and service expiry | FRS v0.3 §3.7 |
| Contract | User type without KGID mandate; still requires official email, office, role, ID proof | FRS v0.3 §3.7 |
| Login name | Official email ID; duplicates rejected | FRS v0.3 §3.7 |
| Is Active | Enable / disable flag for user, role, group or (SRO) office | FRS v0.3 |
...
Kaveri 2.0 provides an admin console for User Management. Administrators (typically KPMU / nominated department admins) create office types and offices in hierarchy IGRO → DRO (and Additional DRO) → SRO (and Additional SRO), define posts and roles, map roles (and groups) to modules / sub-modules / functions, and create users with official email as login. On create, a password is generated and login + password are sent by SMS.

As-Is administration is **create / edit / enable-disable** oriented. There is no first-class Transfer, Relieving, In-charge or Additional charge workflow in FRS v0.3. DSC identity for digital sign is not bound to active post occupancy. Allocation dropdowns in other modules often hold stale or unscoped name lists. SRO Admin powers are inconsistent between FRS §1 (map FDA / SDA / DEO) and FRS §6 (view users / view privileges only). Privilege detail is deferred to UserMapping_v0.1_Annexure, which is not packaged with the FRS PDF.

**As-Is systems:**

| System | Role | Pain points |
|--------|------|-------------|
| Kaveri 2.0 User Management | Office, post, role, user, group, mapping screens | Incomplete lifecycle; SMS-only credential; weak jurisdiction enforcement in consuming modules |
| Kaveri 2.0 module dropdowns (Firm, 68(2), Digital Sign, Dashboard, etc.) | Allocation and identity display | DEO / in-charge / SR names missing or wrong office |
...
> **Source of truth for masters and admin functions:** Kaveri 2.0 User Management FRS v0.3 (keep and harden). **Source of truth for 3.0 lifecycle additions:** programme plan Sr.10 (login, transfer, relieving) and ServiceDesk UM-PP-01 to UM-PP-10. Swimlanes for administration are **Admin (in span of control)**, **System**, and **User (subject of the change)**.
...
| SRO Admin (SR) | FDA / SDA / DEO of that SRO — **Add/Assign if OQ-UM-02 confirms**; otherwise View only as FRS §6 | Other SROs; DR/SR users |
...
| 2 | Upload GO to create new post | Admin | Non-mandatory in FRS; Should in 3.0 |
...
Maintain the catalogue of roles used on posts and users. Example names from FRS: IGR; DIGR Law & Administration / Audit / Intelligence / Vigilance / CVC / Enforcement; AIGR Computers / Administration / Audit; Law Officer; Accounts Superintendent; Superintendent Audit; Statistical Inspector; SR Administration / SR Computers; FDA; SDA; DR; SR; plus DEO and HQA as used in admin sections.
...
Effective privilege for a user = **union of Role mappings**, then **Group mappings override** where a group mapping exists for the same function (FRS rule; confirm OQ-UM-06).
...
| 17 | Force password change on first login | System | 3.0 addition vs FRS SMS-only |
...
| 2 | Filter by Office Type: IGRO = all department users (if authorised); DRO = that DRO + child SRO users; SRO = that SRO only | System | Matches FRS §3.8 |
...
FRS notes this may be handled by the development team; in 3.0 it remains a **controlled admin** function with the same tree UI as role mapping.
...
Maintained so RBAC has a stable catalogue. Examples from FRS: Modules = MIS Reports, CVC, Document Registration, User Management, Dashboard; Sub-modules e.g. Statutory Reports; Functions e.g. View, Print, Download.
...
FRS v0.3 only specifies SMS of generated password on create. Kaveri 3.0 **Must** close UM-PP-01.
...
These workflows are **new versus FRS v0.3** and are Phase 1 Must (programme Sr.10).
...
| C-07 | **Maker-checker + immutable audit** on user, mapping, transfer, privilege | Not in FRS body; 3.0 compliance |
...
| C-10 | Rebuild **privilege matrix** as controlled annexure | FRS annexure missing from PDF |
...
> **Convention:** Req ID `FR-UM-###`. Priority: Must / Should / Could. Trace to FRS v0.3, programme plan or ServiceDesk in the RTM.
...
| FR-UM-039 | GO upload for new post shall be supported and optional (FRS); warning if missing | Should | |
...
| FR-UM-094 | Phone, Role ID, Is Active editable on standard Edit | Must | FRS §3.8 |
...
| Req ID | Requirement | Priority | Acceptance criteria |
|--------|-------------|----------|---------------------|
| FR-UM-176 | SMS (and email if available) on user create with login name | Must | Password via SMS as FRS; not echoed in UI after first display |
...
|---------|-------------|--------|-------------------|
| BR-UM-001 | Office hierarchy is IGRO → DRO (and Additional DRO) → SRO (and Additional SRO) | FRS §3.1–3.2 | Hard stop |
| BR-UM-002 | Data access is restricted by Office ID for SRO users and by Jurisdiction ID for district-scoped roles | FRS §3.7 | Hard filter on UI and API |
| BR-UM-003 | Official email is unique login name | FRS §3.7 | Reject duplicate |
| BR-UM-004 | KGID is mandatory and unique for Government Appointed users | FRS §3.7 | Hard stop |
| BR-UM-005 | DDO code is mandatory for DRO and SRO | FRS §3.2 | Hard stop |
| BR-UM-006 | New office (non-migrated) requires Government Order | FRS §3.2 | Hard stop |
| BR-UM-007 | Is Active controls enable/disable of user, role, group; SRO office disable is explicit | FRS | Login and dropdowns honour flags |
| BR-UM-008 | Group privilege overrides role privilege for the same function | FRS §2 / §3.11 | Evaluation order: group then role |
| BR-UM-009 | Anywhere Registration enabled on one SRO under a DRO implies remaining SROs under that DRO (with message) | FRS §3.2 | Coordinated update + warning |
| BR-UM-010 | Short name is unique and used in registration triplet — change is dual-controlled | FRS §3.2; UM-PP-06 | Dual control |
| BR-UM-011 | Name on standard user edit is locked | FRS §3.8 | Fields read-only |
...
| BR-UM-015 | Login blocked if user inactive, locked, relieved, or Government Appointee past service expiry | FRS + 3.0 | AuthN |
...
| BR-UM-020 | Admins act only inside span of control | FRS §4–6 | 403 outside span |
...
| BR-UM-023 | Catalogue add is restricted to controlled catalogue-admin role | FRS §3.12 | RBAC |
| BR-UM-024 | Disable of role/group/type blocked while in use | FRS edit screens | Referential check |
| BR-UM-025 | Kannada labels for office type, office, role and group are taken from master Kannada fields | FRS bilingual | UI |
...
| Create / list Office Type | Master | KPMU / IGR | FRS §3.1 |
| Add / list Office | Hierarchy, GO, DDO, anywhere registration | KPMU / IGR | FRS §3.2 |
| Add Post | Role + office occupancy slot | Admin in span | FRS §3.3 |
| Add / View / Edit Role | Role catalogue | KPMU / IGR | FRS §3.4–3.5 |
| Map Role to functions | RBAC tree | KPMU / IGR | FRS §3.6 |
| Add User | Full form incl. Govt vs Contract, photo, ID | Admin in span | FRS §3.7 |
| View / Edit User | Filter IGRO/DRO/SRO | Admin in span | FRS §3.8 |
| Create / View / Edit Group | Group master | KPMU / IGR | FRS §3.9–3.10 |
| Map Group to functions | Override tree | KPMU / IGR | FRS §3.11 |
| Add Module / Sub-module / Function | Catalogue | Catalogue admin | FRS §3.12 |
...
**Wireframe links:** Kaveri 2.0 FRS v0.3 UI mock-ups (Create Office Type, Add User, Map Role/Group, Add Module/Function) are the visual baseline; Kaveri 3.0 UX shall add Transfer, Relieving, In-charge, DSC bind, checker inbox and occupancy dashboard. Prototype URLs TBD.
...
|-------------|-----------|---------|-------|--------|
| Khajane-II / DDO mapping table | Inbound | DDO codes for DRO/SRO; payment office identity | Treasury / Arch | Existing FRS rule |
...
| PAN verification API | Outbound | Optional PAN check on Add User | Integration | FRS optional |
...
|-------|-----------------------------------|
| DM-P1-01 | Office / SRO / district masters from Kaveri 1.0/2.0 — volume, duplicate short names, missing DDO |
| DM-P1-02 | Users, roles, transfer/relieving history — agreed depth; password re-issue vs hash migrate |
| Privilege matrix | Rebuild from UserMapping annexure or live Kaveri 2.0 mapping extract |
...
|--------|---------------|---------------------|-------------|-----------|--------------|--------|
| FR-UM-001 | FRS §1 | Admin hierarchy KPMU / IGR / DIGR / DR / SR | 8.1 | Admin Dashboard | TC-UM-___ | Draft |
| FR-UM-007 | FRS §3.7; UM-PP-07 | Hard jurisdiction filter | 8.1 | All admin lists | TC-UM-___ | Draft |
| FR-UM-019 | FRS §3.2 | Office hierarchy IGRO→DRO→SRO | 8.3 | Add Office | TC-UM-___ | Draft |
| FR-UM-020 | FRS §3.2 | GO mandatory for new office | 8.3 | Add Office | TC-UM-___ | Draft |
| FR-UM-022 | FRS §3.2; Khajane | DDO mandatory DRO/SRO | 8.3 | Add Office | TC-UM-___ | Draft |
| FR-UM-026 | FRS §3.2 | Anywhere Registration sibling warning | 8.3 | Add Office | TC-UM-___ | Draft |
| FR-UM-041 | UM-PP-02 | Allocation reads active occupancy | 8.4 | Consuming dropdowns | TC-UM-___ | Draft |
| FR-UM-056 | FRS §3.6 | Map role to functions | 8.6 | Map Role | TC-UM-___ | Draft |
| FR-UM-066 | FRS §3.7 | Add User in span | 8.7 | Add User | TC-UM-___ | Draft |
| FR-UM-069 | FRS §3.7 | KGID mandatory Govt Appointee | 8.7 | Add User | TC-UM-___ | Draft |
| FR-UM-070 | FRS §3.7 | Unique official email login | 8.7 | Add User | TC-UM-___ | Draft |
| FR-UM-081 | FRS §3.7 | SMS login and password on create | 8.7 | Add User | TC-UM-___ | Draft |
| FR-UM-091 | FRS §3.8 | View users by IGRO/DRO/SRO filter | 8.8 | View/Edit User | TC-UM-___ | Draft |
| FR-UM-106 | FRS §3.11 | Group privilege overrides role | 8.9 | Map Group | TC-UM-___ | Draft |
...
| BR-UM-008 | FRS group override | Group overrides role | 9 | Map Group | TC-UM-___ | Draft |
...
- Functional Requirement Specification — User Management Module, KAVERI 2.0 Online Services (CSG), v0.3 — `Requirement Discussions/Kaveri2.0/User Management KAVERI 2.0_v0.3.pdf`
...
- UserMapping_v0.1_Annexure — referenced by FRS; **not attached to the PDF**; locate or rebuild before UAT
...
Actions listed in FRS §§4–6, reconciled for Kaveri 3.0. Cells: Y = allowed in span; N = not allowed; C = confirm (OQ-UM-02).
...
| Edit / Add Group | Y | Y | N (view in FRS DIGR list) | N | N |
...
FRS DIGR list omits Edit Group / Add Group / Edit Roles / Add Role (view only). FRS SRO list is only View users and View privileges, while FRS §1 says SR maps FDA/SDA/DEO — **freeze via OQ-UM-02**.

## Appendix C — Seed office types, roles and ID-proof types

**Office types:** IGRO; DRO; SRO; Additional DRO; Additional SRO.

**Seed roles (from FRS, plus Phase 1 consumers):** Super Admin (KPMU); IGR; DIGR Law & Administration; DIGR Audit; DIGR Intelligence; DIGR Vigilance; DIGR CVC; DIGR Enforcement; AIGR Computers; AIGR Administration; AIGR Audit; Law Officer; Accounts Superintendent; Superintendent Audit; Statistical Inspector; District Registrar (DR); Head of Accounting (HQA); Sub-Registrar (SR); SR Administration; SR Computers; FDA; SDA; DEO; Catalogue Admin.

**ID proof types (FRS §3.7):** PAN; Passport; Driving licence; Bank pass book; Matriculation certificate; Degree of a recognised educational institution; Ration card; Aadhaar Number; Voter ID (EPIC). Validate number format per type.

**User types:** Government Appointed; Contract.

**Group templates (FRS):** IGR Office; DR Office; SR Office.
...
|------|----------|-----------------|-------------|-----|
| OQ-UM-01 | Retain FRS admin hierarchy (KPMU / IGR / DIGR / DR / SR) for 3.0? Add AIGR Computers as statewide IT admin? | Keep as-is / simplify / add AIGR Computers | PO, IGR | |
| OQ-UM-02 | What can SR Admin do — only view, or also add/map FDA–SDA–DEO? | Resolve FRS §1 vs §6 | PO, Domain Expert | |
...
| OQ-UM-06 | Group privilege override of Role — keep, reverse, or remove groups? | FRS says group overrides role | PO, Security | |
...
| OQ-UM-12 | Rebuild UserMapping annexure as controlled matrix? | Required; missing from PDF | BA, AIGR Computers | |
| OQ-UM-13 | Anywhere Registration: auto-enable all sibling SROs or only warn? | FRS says message + enable remaining | DE, PO | |
...
| DEC-UM-002 | FRS v0.3 functions (office type/office/post/role/user/group/mapping/catalogue) are **kept** as baseline | Draft | PO (to confirm) | §7.2–7.10 |
