# -*- coding: utf-8 -*-
"""Build ERD_User_Management_v2.0.docx — logical ERD for User Management BRD v4.16.

Standalone deliverable; does not modify BRD_User_Management_v4.16.docx.
"""
...
PNG_OVERVIEW = BASE / "ERD_User_Management_v2.0_overview.png"
PNG_ORG = BASE / "ERD_User_Management_v2.0_org_posts.png"
PNG_ID = BASE / "ERD_User_Management_v2.0_identity_occupancy.png"
PNG_RBAC = BASE / "ERD_User_Management_v2.0_rbac_runtime.png"
DST = BASE / "ERD_User_Management_v2.0.docx"
...
def finish(img: Image.Image, draw: ImageDraw.ImageDraw, fonts: dict, path: Path) -> None:
    draw.text(
        (24, img.height - 32),
        "KAVERI 3.0  ·  DSR  ·  Logical ERD v2.0  ·  Companion to BRD User Management v4.16  ·  Not part of the BRD",
        font=fonts["sub"],
        fill=MUTED,
    )
...
def render_overview() -> None:
    img, draw, fonts = canvas(
        ...
        "Single User Master and Role Master  ·  DSR privileges flow occupancy → Post–Role → Module Function  ·  BRD v4.16",
        2400,
        1280,
    )
    boxes: dict[str, Box] = {}
    ...
    draw_rel(draw, b["USER_MASTER"], "s", b["USER_SESSION"], "w", "1", "0..1", "FR-UM-076", r, via=[(185, 900), (1270, 900)])
...
def render_identity() -> None:
    img, draw, fonts = canvas(
        ...
        "Username unique (Citizen preferred / KGID)  ·  Email and mobile not unique  ·  Additional charge is session-only (FR-UM-053)  ·  No password entity",
        2400,
        1280,
    )
    boxes: dict[str, Box] = {}
    ...
    add(Box("DOM", "OFFICIAL_EMAIL_DOMAIN", ["PK domain", "is_active", "FR-UM-064 allow-list"], 1660, 170, "id", 260))
...
def build_docx() -> None:
    ...
    add_table(
        ...
        [
            ...
            ("Version", "2.0"),
            ("Status", "Draft / companion to BRD v4.16"),
            ("Project", "KAVERI 3.0 — Department of Stamps and Registration"),
            ("Module", "User Management & RBAC"),
            ("Source BRD", "Finalized BRD/User Management/BRD_User_Management_v4.16.docx"),
    ...
    add_figure(PNG_OVERVIEW, "Figure 1. Overview — all logical entities and cardinalities (BRD v4.16)")
    ...
    doc.add_paragraph(
        ...
        "For DSR: assigned_occupancy_id is the FR-UM-052 post for the session. Additional charge (FR-UM-053) is "
        "session-only — it does not create a POST_OCCUPANCY row and does not require an order. While "
        "additional charge is set, Module Function claims come only from that subordinate post (FR-UM-038)."
    )
    add_table(
        doc,
        ["Attribute", "Type / notes", "Constraints"],
    ...
    add_table(
        ...
        [
            ...
            (
                ...
                "1..* sanctioned posts",
                "None (FR-UM-065)",
                "FR-UM-052 assigned post; FR-UM-053 additional charge on session only",
            ),
            (
                "Other Department",
                "KGID = Username",
    ...
    add_table(
        ...
        ["Fact", "How obtained"],
        [
            ("Session Module Function claims", "Citizen/OD: USER_ROLE → RMF. DSR without additional charge: assigned occupancy → POST_ROLE_MAP → RMF. DSR with FR-UM-053 active: additional-charge post only (FR-UM-038)."),
            ("Office span", "Walk OFFICE_HIERARCHY descendants of session office (FR-UM-059)."),
            ("Immediate-parent transfer filter", "OFFICER_HIERARCHY_NODE.parent of the target post = actor session post (FR-UM-057, FR-UM-060)."),
            ("Available capacity FR-UM-066(a)", "SANCTIONED_POST.occupied_count < sanctioned_strength."),
            ("Wholly unoccupied FR-UM-066(b)", "occupied_count = 0 (reserved Transfer In counts as occupied)."),
    ...
    doc.add_paragraph(
        "Primary / Secondary role flags — removed; DSR context is assigned post vs additional charge (FR-UM-052 / FR-UM-053)."
    )
    doc.add_paragraph(
        "Separate Citizen vs Department user databases — forbidden; one USER_MASTER and one ROLE_MASTER (FR-UM-016)."
    )

    doc.add_heading("10. Alignment to BRD v4.16", level=1)
    doc.add_paragraph(
        "This ERD is a companion to BRD_User_Management_v4.16.docx. If a functional requirement and this model "
    ...
    run = sig.add_run("Related documents: BRD-K3-UM-001 v4.16 (requirements) · ERD-K3-UM-001 v2.0 (this document).")
    run.italic = True
    run.font.size = Pt(10)
