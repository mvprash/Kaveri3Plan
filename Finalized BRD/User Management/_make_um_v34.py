# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v3.4.docx — vacant subordinate post/role switch after login (hierarchy)."""
...
DST = BASE / "BRD_User_Management_v3.4.docx"
...
def build() -> Document:
    ...
    replace_paragraph_text(
        ...
        "Other Department), OTP-based authentication (no password management), a separate Posts Master "
    ...
    replace_paragraph_text(
        ...
        "Captcha, and Biometrics for departmental users. DSR officers are assigned to vacant sanctioned "
    ...
    scope_in = {
        ...
        18: "OTP-based authentication (login, logout) — Username + OTP + Captcha; biometrics for departmental users; multi-post DSR login post selection; vacant subordinate post switch via hierarchy (FR-53)",
        ...
        22: "Posts Master, Post–Role mapping (one post → many roles), Sanctioned Posts Master; DSR Officers assigned one or more vacant sanctioned posts (no Primary/Secondary); Other Department users one role with optional end date",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                "FR-53",
                "After login and post selection (FR-52), once the DSR Officer is on the home page, the "
                "system shall allow the user to switch the active session post/role to vacant subordinate "
                "posts under the login-selected post, using the DSR Officer Hierarchy Master (FR-43). "
                "Eligibility: for the login-selected Post at the same Office, examine each immediate "
                "child post in the hierarchy; if that child post is vacant at that Office, it shall be "
                "offered as a switch option and the system shall continue one level further under that "
                ...
                "at Office A; if FDA under Sub-Registrar is vacant, switch to FDA is allowed; if SDA under "
                "that vacant FDA is also vacant, switch to FDA or SDA is allowed. On switch, session roles "
                "and Module Function claims shall be recalculated from the switched-to post via Post–Role "
                "mapping. The user shall always be able to switch back to the original login-selected post. "
                "Switch actions shall be audit-logged (actor, from-post, to-post, office).",
                "High",
            ),
        ],
    )
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "roles (for DSR Officers: from the current session post — login-selected or FR-53 switch); "
            ...
            (
                ...
                "current session post (login-selected post or a vacant subordinate post switched to "
                "under FR-53) via Post–Role mapping; for Other Department users, the single assigned "
                "role; for Citizens, assigned Citizen roles.",
                "High",
            ),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Post occupancy end date not enforced", "Medium", "Scheduled job to free expired post occupancies and recalculate roles; audit alert."),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Session post", "The Post + Office context currently active in the DSR session — either the login-selected post or a vacant subordinate post switched to under FR-53; session roles derive from this post via Post–Role mapping"),
            ("Vacant subordinate switch", "Home-page action allowing a DSR Officer to act under a vacant child/descendant post of the login-selected post at the same office, cascading through vacant hierarchy nodes only (FR-53)"),
    ...
    note2 = insert_paragraph_after(
        ...
        "after authentication before entering the application (FR-52). From the home page they may "
        "switch to vacant subordinate posts under that selection using the Hierarchy Master (FR-53).",
        "Normal",
    )
    cat_tbl = insert_table_after(
        ...
        [
            ...
            ("Department users (DSR Officers)", "Officers and staff of DSR — User Category = DSR Officer", "Username + OTP + Captcha + Biometrics; then post selection if multiple posts (FR-52)"),
            (
    ...
    login_post_note = insert_paragraph_after(
        login_post_intro,
        "Applies only to DSR Officers. After Username + OTP + Captcha + Biometrics succeed, the system "
    ...
    login_post_tbl = insert_table_after(
        ...
        [
            ("1", "Authenticate (Username + OTP + Captcha + Biometrics)", "DSR Officer / Auth", "FR-06"),
            ...
            ("6", "Resolve session roles from selected post via Post–Role mapping", "UM / Auth", "Not union of all posts"),
            ("7", "Resolve Module Function claims; enter application home", "UM / Auth", "FR-38, FR-41"),
            ("8", "On home — optionally switch to vacant subordinate post(s) under login post", "DSR Officer / UI", "Hierarchy Master; same office; FR-53"),
        ],
    )

    switch_intro = insert_heading_after(
        login_post_tbl,
        "Vacant subordinate post / role switch (DSR Officers — FR-53):",
        "Normal",
        parent=rbac_parent,
    )
    switch_note = insert_paragraph_after(
        switch_intro,
        "After the officer lands on the home page under the login-selected post, the system shall "
        "offer switch options for vacant posts that sit under that post in the DSR Officer Hierarchy "
        "Master, at the same Office. Cascading: only vacant children are offered; under each vacant "
        "child, vacant grandchildren are also offered, and so on. Occupied posts are not offered and "
        "stop further descent. Roles after switch come from Post–Role mapping of the switched-to post. "
        "The officer may always return to the original login-selected post.",
        "Normal",
    )
    switch_tbl = insert_table_after(
        ...
        ["Step", "Action", "Actor / System", "Notes"],
        [
            ("1", "Land on home under login-selected Post + Office", "DSR Officer", "FR-52 complete"),
            ("2", "Read Hierarchy Master children of login-selected Post", "System", "FR-43"),
            ("3", "For each child at same Office — if vacant, add as switch option", "System", "Vacancy = Occupied < Sanctioned strength (FR-24)"),
            ("4", "If child is vacant, repeat for its children (cascade)", "System", "Stop cascade under occupied posts"),
            ("5", "Show switch list (Post Name + Office); include original login post", "UI", "Example: SR → FDA (vacant) → SDA (vacant)"),
            ("6", "User selects a switch target", "DSR Officer", "Optional; may stay on login post"),
            ("7", "Recalculate session roles via Post–Role mapping for switched-to post", "UM / Auth", "FR-38, FR-47"),
            ("8", "Audit log switch (from-post, to-post, office, actor, timestamp)", "System", "Mandatory"),
        ],
    )
    switch_ex_intro = insert_heading_after(
        switch_tbl,
        "Illustrative example (Office A — Sub-Registrar login):",
        "Normal",
        parent=rbac_parent,
    )
    switch_ex_tbl = insert_table_after(
        switch_ex_intro,
        ["Hierarchy (Office A)", "Vacancy at Office A", "Switch options for Sub-Registrar"],
        [
            ("Sub-Registrar (login post)", "Occupied by self", "Always available (return / stay)"),
            ("→ FDA (Enforcement) under Sub-Registrar", "Vacant", "May switch to FDA (Enforcement)"),
            ("→ → SDA (Enforcement) under FDA", "Vacant", "May switch to SDA (Enforcement) as well"),
            ("→ FDA (Enforcement) under Sub-Registrar", "Occupied by another officer", "FDA not offered; SDA under FDA not offered (cascade blocked)"),
        ],
    )

    sub3 = insert_heading_after(
        switch_ex_tbl, "6.5.3 Posts Master, Post–Role Mapping, and Sanctioned Posts", "Heading 3", parent=rbac_parent
    )
    note3 = insert_paragraph_after(
        ...
        "post (login or vacant subordinate switch — FR-53). "
    ...
    primary_note = insert_paragraph_after(
        ...
        "roles = roles mapped to the current session post via Post–Role mapping. From the home page, "
        "the officer may switch to vacant subordinate posts under that login post using the Hierarchy "
        "Master (FR-53). Division-specific posts "
    ...
    primary_tbl = insert_table_after(
        ...
        [
            (
                ...
                "Roles from Post–Role mapping for current session post (login or FR-53 switch)",
                "Optional per post occupancy",
                "Multi-post → choose at login; vacant subordinates → switch on home",
            ),
            (
                "Other Department",
    ...
    enf_tbl = insert_table_after(
        ...
        [
            ("1", "User authenticates (OTP / OTP+Biometrics as per category)", "User / Auth service", "Credentials verified"),
            ...
            ("1b", "On home — optional switch to vacant subordinate post at same Office", "User / UI", "Hierarchy cascade while vacant (FR-53); may return to login post"),
            ("2", "Load session role(s)", "Auth / UM service", "DSR: roles from current session post via Post–Role map; Other Department: one role"),
    ...
    note7 = insert_paragraph_after(
        ...
        "subordinate post/role switch after login (FR-53). Application Admin shall add, edit, reorder, "
    ...
    hier_seed_intro = insert_paragraph_after(
        ...
        "is Sub-Registrar → FDA (Enforcement) → SDA (Enforcement) so FR-53 switch examples apply:",
        "Normal",
    )
    hier_seed_tbl = insert_table_after(
        ...
        [
            ...
            ("3", "Enforcement — SRO", "POST-SR", "Sub-Registrar", "POST-DIGR-ENF"),
            ("4", "Enforcement — SRO", "POST-FDA-ENF", "FDA (Enforcement)", "POST-SR"),
            ("5", "Enforcement — SRO", "POST-SDA-ENF", "SDA (Enforcement)", "POST-FDA-ENF"),
            ("4", "Enforcement — SRO", "POST-DEO", "DEO", "POST-SR"),
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v3.4"
