...
| **Module** | User Management (departmental login, office, post, role, group and access rights) |
...
| ID | Title | Link |
|----|--------|------|
| BRD-K3-UM-001 | This document | Finalized BRD/User Management/BRD_User_Management_v1.3.docx |
...
This Business Requirements Document specifies the departmental **User Management** requirements for Kaveri 3.0. It covers **login, roles, office mapping, transfer, relieving and access rights** for officers and staff of the Department of Stamps & Registration.

The Kaveri 3.0 User Management module is the **central departmental login, role and office control used by all modules**. It shall provide delegated administration (KPMU / IGR / DIGR / DR / SR), bilingual office and role masters, post-based occupancy, role- and group-based privileges, verified officer identity (KGID, official email, mobile), transfer and relieving, in-charge occupancy, DSC binding to the active SR (or other signing post), immutable audit of privilege changes, and a single master that allocation, MIS, OTP and digital-sign consumers must read.

**Success criteria (measurable):**

- Departmental users authenticate and receive menus solely from this module.
...
**Module boundary:** This BRD specifies User Management for Kaveri 3.0 for all consuming modules. Citizen authentication / eKYC account lifecycle is **out of this BRD** unless the Product Owner promotes it (see OQ-UM-11).
...
- Role and group access: map Role and Group to Module / Sub-module / Function (view, add, edit, print, scan, download, approve, sign, and other function verbs as catalogued).
...
- Seed and maintain the departmental roles required by consuming modules (including Marriage: SR, DEO, FDA, SDA, DR, IGR and related) so those modules’ access rights can be enforced from this module.
...
| Access model | Who | What they do | In scope? |
|--------------|-----|--------------|------|
...
The function catalogue **Must** include User Management functions plus consuming modules as they are specified (Marriage Online/Offline, CC, scanning, Khajane payment operations, dashboard/MIS, admin audit/export, and later Document / Firm / EC and others).
...
These workflows are **new versus As-Is** and are required for Kaveri 3.0.
...
| FR-UM-049 | DEO, SR, FDA, SDA, DR, IGR, HQA shall exist as first-class roles | Must | Marriage DEO console authorises DEO role only |
...
| FR-UM-120 | Function catalogue shall include User Management, Marriage, CC, Scanning, Payment, Dashboard/MIS, Audit export, and shall be extensible for other Kaveri 3.0 modules | Must | Mapping screens list these modules; new modules can be added |
...
**Go-live gate:** User Management login / transfer / relieving operational, with consuming-module roles (including Marriage SR, DEO) bound to occupancy.
...
- Project Plan Kaveri 3.0 Programme v0.4 — User Management Sr.10–11; DM-P1-02
- Marriage BRD v1.6 (section pattern and consuming access-rights / DSC needs) — `Finalized BRD/Marriage/RFP/BRD_Marriage_v1.6.docx`
...
| OQ-UM-09 | Jurisdiction enforcement on **all** modules from go-live, or phased by consuming module? | Recommend hard filter on all APIs from day one | Arch, PO | |
...
|--------|----------|------|----------|--------|
| DEC-UM-001 | User Management = central departmental login, role and office control used by all modules | Programme plan v0.4 | Steering / PO | This BRD scope |
