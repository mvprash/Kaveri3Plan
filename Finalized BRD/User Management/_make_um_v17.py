# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v1.7.docx from template with OTP-only auth requirements."""
...
DST = BASE / "BRD_User_Management_v1.7.docx"
...
def build() -> Document:
    ...
    replace_paragraph_text(
        ...
        "OTP-based authentication (no password management), and the DSR division and role "
        "structure, and serves as the agreed basis for design, development, testing, and sign-off.",
    )
    replace_paragraph_text(
        doc.paragraphs[14],
        "KAVERI 3.0 requires a centralized mechanism to manage user identities, roles, and access "
        "permissions across citizen and departmental users. Authentication shall be passwordless — "
        "Username with OTP and Captcha for citizens, and Username with OTP, Captcha, and Biometrics "
        "for departmental users. Password management is explicitly out of scope.",
    )
    ...
    scope_in = {
        ...
        20: "User profile management (view, update, deactivate)",
        21: "Administrative user management (create, edit, suspend, deactivate users)",
        22: "Audit logging of user-related activities",
    }
    for idx, text in scope_in.items():
        replace_paragraph_text(doc.paragraphs[idx], text)
    remove_paragraph(doc.paragraphs[23])
    ...
    # Update all template tables BEFORE inserting new ones (indices 0–12)
    replace_table_rows(
        doc.tables[0],
        ["Document Control", ""],
        [
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Prabhakar Naik", "Domain Expert", "Validates DSR roles and organizational structure"),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("FR-17", "The system shall allow administrators to assign one or more roles to Department users.", "High"),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("FR-21", "The system shall allow administrators to search and filter users by category, role, division, and status.", "Medium"),
    ...
    # Capture RBAC FR table element before inline table inserts alter doc.tables indices
    rbac_fr_tbl_el = doc.tables[7]._tbl

    # Insert division/role and user-category tables under Section 6.5
    rbac_heading = next(p for p in doc.paragraphs if p.text.strip() == "6.5 Role-Based Access Control (RBAC)")

    # Place RBAC FR table (FR-16–FR-19) immediately after §6.5 heading
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v1.7"
