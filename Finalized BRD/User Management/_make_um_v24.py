# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v2.4.docx — Module Master and Role-to-Module mapping (§6.5.6)."""
...
DST = BASE / "BRD_User_Management_v2.4.docx"
...
def build() -> Document:
    ...
    replace_paragraph_text(
        ...
        "end date, a Module Master (Document Registration, Marriage Registration, Encumbrance Search, "
        "Certified Copy, and others), Role-to-Module privilege mapping, and dedicated user-creation "
        "workflows. It serves as the agreed basis for design, development, testing, and sign-off.",
    )
    replace_paragraph_text(
        doc.paragraphs[14],
        ...
        "date that deactivates the user when reached. Application modules (e.g. Document Registration, "
        "Marriage Registration, Encumbrance Search, Certified Copy) are maintained in a Module Master "
        "and mapped to roles — DSR organisational roles are not named after modules. Password "
    ...
    scope_in = {
        ...
        20: "Module Master and Role-to-Module mapping (Document Registration, Marriage Registration, Encumbrance Search, Certified Copy, and other modules)",
        21: "Sanctioned posts master and Primary/Secondary role assignment for DSR Officers; single role for Other Department users with optional account end date",
    ...
    for p in doc.paragraphs:
        if p.text.strip() == "Enforce role-based access control to protect sensitive data and functionality.":
            insert_paragraph_after(
                p,
                "Support Module Master and Role-to-Module mapping so organisational roles gain access to Document Registration, Marriage Registration, Encumbrance Search, Certified Copy, and other modules.",
                "List Paragraph",
            )
            break
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                "FR-36",
                "The system shall maintain a Module Master of application modules (including Document "
                "Registration, Marriage Registration, Encumbrance Search, Certified Copy, and others "
                "listed in Section 6.5.6). Modules are not DSR organisational roles; they define "
                "functional areas to which roles are mapped.",
                "High",
            ),
            (
                "FR-37",
                "The system shall allow authorised administrators to map one or more modules (and "
                "module functions such as view, add, edit, approve, sign, print, download) to roles "
                "in the Role Master. Access to a module is granted only through Role-to-Module mapping.",
                "High",
            ),
            (
                "FR-38",
                "A user's effective module access shall be the union of modules mapped to all roles "
                "currently assigned to that user (Primary and Secondary for DSR Officers; the single "
                "assigned role for Other Department users; Citizen roles for Public users).",
                "High",
            ),
            ("FR-18", "The system shall restrict access to features and data based on the user's assigned role(s) and the Role-to-Module mappings defined for those roles.", "High"),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Module Master", "Catalogue of application modules (e.g. Document Registration, Marriage Registration, Encumbrance Search, Certified Copy)"),
            ("Role-to-Module mapping", "Association of one or more modules and module functions to a role; determines what a role-holder can access"),
    ...
    citizen_tbl = insert_table_after(
        ...
        [
            ...
            ("Document Registration Applicant", "Initiate and track online / offline document registration applications"),
    ...
    sub6 = insert_heading_after(
        primary_tbl, "6.5.6 Module Master and Role-to-Module Mapping", "Heading 3", parent=rbac_parent
    )
    note6 = insert_paragraph_after(
        sub6,
        "DSR organisational roles (SR, FDA, DEO, etc.) are not named after application services. "
        "Application capabilities such as Document Registration, Marriage Registration, Encumbrance "
        "Search, and Certified Copy are maintained in a Module Master and mapped to roles. A user "
        "gains module access only through the Role-to-Module mappings of the role(s) assigned to them.",
        "Normal",
    )
    mod_intro = insert_paragraph_after(note6, "Seed modules in the Module Master:", "Normal")
    mod_tbl = insert_table_after(
        mod_intro,
        ["Module", "Description", "Typical consumers"],
        [
            ("Document Registration", "Online and offline registration of documents", "Citizen applicants; DSR SRO staff; Other Department verifiers"),
            ("Marriage Registration", "Hindu Marriage and Special Marriage registration", "Citizen applicants; DSR SRO / DEO / SR"),
            ("Encumbrance Search", "Search and issue of Encumbrance Certificate (EC)", "Citizen applicants; DSR office staff"),
            ("Certified Copy", "Application and issue of certified copies of registered documents", "Citizen applicants; DSR office staff"),
            ("Stamp Duty / Payments", "Stamp duty assessment, challan, and payment reconciliation", "Citizens; DSR; Treasury Other Department roles"),
            ("Firm / Society Registration", "Firm and society related registration services", "Citizen applicants; DSR staff"),
            ("User Management", "Identity, roles, posts, modules, and privilege administration", "Application Admin; designated admin roles"),
            ("MIS / Dashboards", "Operational and management reporting", "DSR officers; Other Department MIS viewers"),
        ],
    )
    map_intro = insert_heading_after(
        mod_tbl,
        "Illustrative Role-to-Module mapping (seed — configurable by Application Admin):",
        "Normal",
        parent=rbac_parent,
    )
    map_tbl = insert_table_after(
        map_intro,
        ["Role (from Role Master)", "Role Category", "Mapped Modules", "Sample Functions"],
        [
            ("Citizen", "Citizen", "Document Registration; Marriage Registration; Encumbrance Search; Certified Copy; Stamp Duty / Payments", "Apply, view own applications, pay, download"),
            ("Marriage Applicant", "Citizen", "Marriage Registration", "Apply, upload documents, track status"),
            ("Document Registration Applicant", "Citizen", "Document Registration", "Apply, track, pay"),
            ("Certified Copy / EC Applicant", "Citizen", "Encumbrance Search; Certified Copy", "Apply, pay, download"),
            ("Sub-Registrar (SR)", "DSR", "Document Registration; Marriage Registration; Encumbrance Search; Certified Copy", "View, approve, sign, print"),
            ("DEO", "DSR", "Document Registration; Marriage Registration", "Add, edit, scan, view"),
            ("FDA", "DSR", "Document Registration; Certified Copy; Encumbrance Search", "View, add, edit, print"),
            ("SDA", "DSR", "Document Registration; Encumbrance Search", "View, add, print"),
            ("DRO", "DSR", "Document Registration; Marriage Registration; MIS / Dashboards", "View, approve (district span), MIS"),
            ("Revenue Verification Officer", "Other Department", "Document Registration", "View, verify revenue particulars"),
            ("Treasury Payment Verifier", "Other Department", "Stamp Duty / Payments", "View, verify payment status"),
            ("Police Enquiry Officer", "Other Department", "Document Registration; Certified Copy", "View, respond to enquiry"),
            ("Read-only MIS Viewer", "Other Department", "MIS / Dashboards", "View only"),
        ],
    )
    insert_heading_after(
        map_tbl,
        "Application Admin shall maintain Role-to-Module mappings. Fine-grained function lists per "
        "module may be extended as consuming-module BRDs are finalised. Changes to mappings shall "
        "be audit-logged (FR-22, FR-37).",
        "Normal",
        parent=rbac_parent,
    )
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v2.4"
