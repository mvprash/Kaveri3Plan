...
| ID | Title | Link |
|----|--------|------|
| BRD-K3-UM-001 | This document | Finalized BRD/User Management/BRD_User_Management_v1.5.docx |
...
- 7.4 Department user (officer) onboarding and lifecycle
- 7.5 Role-Based Access Control and session management
- 7.6 Authentication and credential management
- 7.7 System administration and configuration
- 7.8 Automated letter generation
- 7.9 Reporting and MIS
- 7.10 Status models
...
The module manages the **lifecycle, authentication, authorization, and hierarchy mapping** of all users of the Kaveri platform. It serves two distinct user categories:
...
| **Department users (Officers)** | Officers and staff of DSR (IGR, DIGR, AIGR, SR, FDA, SDA, etc.) | Created **only** by designated admin roles (e.g. AIGR Admin) — configurable by Application Admin | Username + OTP + Captcha + **Biometrics** |
...
- **Controlled department user provisioning** — only roles designated by the Application Admin (e.g. AIGR Admin, Super Admin) may create officer accounts.
...
- **Split passwordless authentication** — citizens: Username + OTP + Captcha; officers: Username + OTP + Captcha + Biometrics (mandatory).
...
- Department users are created only by configured designated admin roles and mapped to a sanctioned post.
- Officers authenticate with Username + OTP + Captcha + Biometrics; no password option exists.
...
- **Department users (Officers):** provisioning by designated admin roles only, mapping to sanctioned posts, passwordless login (Username + OTP + Captcha + Biometrics).

**Core capabilities:**

- Sanctioned **posts master** (IGR, DIGR, AIGR, Sub-Registrar, First Division Assistant, Second Division Assistant, DRO, HQA, and other DSR posts) with sanctioned strength per office.
- Mapping of department users to **pre-defined sanctioned posts only**.
- Configurable **designated admin roles** authorised to create department users (e.g. AIGR Admin — set by Application Admin).
...
| A-02 | **Username** is the login identifier for both citizens and department users | Security, PO |
| A-03 | OTP is delivered to the user's registered mobile and/or email within 5 seconds | Ops, Arch |
| A-04 | Biometric capture devices and UIDAI / department approvals are in place before department user login is enabled | Security, Legal |
| A-05 | Official letter templates for Joining and Relieving are provided before UAT | Domain Expert |
| A-06 | Application Admin configures which roles may create department users (default includes AIGR Admin) | PO |
...
- **Department users** must complete Username + OTP + Captcha + Biometrics on every login.
- Department users may be assigned **only** to posts in the sanctioned posts master.
- Department user accounts may be created **only** by roles in the designated-creator list (configurable).
- OTP delivery within **5 seconds** (NFR-UM-02).
- Biometric data re-registered every **5 years** for department users (REQ-UM-013).
...
| **Designated admin (e.g. AIGR Admin)** | Department | Officer role authorised to create department users | Create officer accounts; map users to sanctioned posts |
...
| **Department user (Officer)** | Department | Any DSR officer or staff with a Kaveri login | Log in (Username + OTP + Captcha + Biometrics); select session role |
| **Security / audit reviewer** | Department | Internal audit, AG, STQC | Inspect user, post occupancy and privilege history |

**RACI (summary):**

| Step | Application Admin | Designated admin (e.g. AIGR Admin) | IGR / DIGR | Citizen | Officer |
|------|-------------------|-------------------------------------|------------|---------|---------|
| Citizen self-registration (instant) | — | — | — | R | — |
| Configure designated creator roles | A/R | I | I | — | — |
| Create department user | C | A/R | C | — | I |
| Map officer to sanctioned post | C | A/R | C | — | I |
| Manage sanctioned posts master | A/R | C | C | — | — |
| Citizen login (Username+OTP+Captcha) | — | — | — | R | — |
| Officer login (+Biometrics) | — | — | — | — | R |
...
| Department user (Officer) | An officer or staff member of DSR with a Kaveri departmental login | §7.1 |
| Username | Login identifier for both user categories (citizen-chosen or system-assigned per policy) | REQ-UM-025, REQ-UM-026 |
| Instant registration | Citizen account created immediately on self-registration without approval workflow | REQ-UM-001 |
| Designated creator role | A department role authorised by Application Admin to create department user accounts (e.g. AIGR Admin) | REQ-UM-022 |
...
| Passwordless Authentication | Login without passwords — citizens: Username + OTP + Captcha; officers: + Biometrics | REQ-UM-025, REQ-UM-026 |
...
| UM-PP-06 | No biometric mandate for officers | Weak officer authentication | REQ-UM-026 |
...
| Department console | Department user (Officer) | DSR officers and staff | **Designated admin roles only** (REQ-UM-022) | Username + OTP + Captcha + Biometrics (REQ-UM-026) | Yes |
| Application administration | Application Admin | KPMU / Super Admin | N/A (bootstrap) | Officer auth flow | Yes |

Citizens and department users are **separate identity domains** with distinct registration paths, authentication flows, and privilege models. A single physical person may hold both a citizen account and an officer account, but these are separate records.
...
#### 7.4.1 Department user creation (designated admin only)
...
*(Applies to department users only.)*

#### 7.5.1 Session role selection
...
#### 7.5.3 Peer / subordinate role editing
...
#### 7.6.1 Citizen (public user) — passwordless login
...
#### 7.6.2 Department user (officer) — passwordless login
...
| 10 | Access granted; proceed to session role selection (§7.5.1) | System | |

### 7.7 System administration and configuration

#### 7.7.1 Configure designated creator roles

| # | Step | Lane | Notes |
|---|------|------|-------|
| 1 | Application Admin opens Configuration → Designated Creator Roles | Admin | REQ-UM-022 |
| 2 | Select role(s) authorised to create department users (e.g. AIGR Admin, Super Admin) | Admin | Configurable list |
| 3 | Save | System | Audit event |

#### 7.7.2 Dynamic role and hierarchy configuration
...
| 2 | Report shows logins by user category (citizen / officer) and session role | System | REQ-UM-021 |
| 3 | Sanctioned post occupancy report (vacant / filled) | Admin | REQ-UM-023 |
| 4 | Export CSV / PDF | Admin | |

### 7.10 Status models

#### 7.10.1 Citizen account status
...
#### 7.10.2 Department user account status
...
#### 7.10.3 Sanctioned post occupancy status
...
| REQ-UM-022 | **Department users (officers):** accounts shall be created **only** by users holding a **designated creator role** (e.g. AIGR Admin). The list of designated creator roles shall be **configurable by the Application Admin** | Must | Non-designated role cannot access Add Department User; AIGR Admin can when configured |
...
| REQ-UM-024 | Department users shall be mapped **only** to posts defined in the sanctioned posts master. Assignment to a post not in the master or exceeding sanctioned strength shall be blocked | Must | Unlisted post rejected; over-capacity rejected |
...
| REQ-UM-026 | **Department users (officers)** shall authenticate using **Username**, **OTP**, **Captcha**, and **Biometrics** (all four mandatory). Access granted only when all four succeed | Must | Officer login blocked if any factor fails; biometric mandatory |
| REQ-UM-012 | Biometric capture and verification shall be integrated for department user login | Must | Biometric step present in officer login flow |
| REQ-UM-013 | Department users must re-register biometrics every five (5) years | Must | Login blocked until re-registration when expired |
...
| Req ID | Requirement | Priority | Acceptance criteria |
|--------|-------------|----------|---------------------|
| REQ-UM-020 | Daily report of users logged in, by user category | Must | Citizen and officer counts per day |
...
|---------|-------------|--------|-------------------|
| BR-UM-001 | Two user categories: Public (Citizen) and Department (Officer) — separate identity domains | §7.1 | Hard separation |
| BR-UM-002 | Instant registration applies to **citizens only** | REQ-UM-001 | No approval workflow for citizens |
| BR-UM-003 | Department users created **only** by designated creator roles (configurable) | REQ-UM-022 | 403 for non-designated roles |
| BR-UM-004 | Department users mapped **only** to sanctioned posts in the posts master | REQ-UM-024 | Hard stop |
| BR-UM-005 | Sanctioned strength per office cannot be exceeded | REQ-UM-023 | Block over-capacity assignment |
| BR-UM-006 | Primary Role mandatory for officers; no end date | REQ-UM-002 | Hard stop |
| BR-UM-007 | Secondary Roles require end date; auto-expire | REQ-UM-003 | Scheduled job |
| BR-UM-008 | Citizen auth: Username + OTP + Captcha only (no biometrics) | REQ-UM-025 | AuthN gate |
| BR-UM-009 | Officer auth: Username + OTP + Captcha + Biometrics (all mandatory) | REQ-UM-026 | AuthN gate |
| BR-UM-010 | No password authentication in KAVERI 3.0 | REQ-UM-010 | Password UI removed |
| BR-UM-011 | Biometric re-registration every 5 years for officers | REQ-UM-013 | Login block |
| BR-UM-012 | All user-management actions audited (timestamp, IP, actor) | NFR-UM-01 | Immutable audit |
...
| Officer login | Username + Captcha + OTP + Biometrics | Officer | Department | REQ-UM-026 |
| Biometric re-registration | 5-year mandatory update | Officer | Department | REQ-UM-013 |
| Session role picker | Select role after login | Officer | Department | REQ-UM-007 |
| Role switcher | Change session role | Officer | Department | REQ-UM-008 |
| Add Department User | Create officer (designated admin only) | Designated admin | — | REQ-UM-022 |
| Map to sanctioned post | Assign officer to vacant post | Designated admin | — | REQ-UM-024 |
| Posts master | Manage sanctioned posts and strength | Application Admin | — | REQ-UM-023 |
| Designated creator config | Configure who may create officers | Application Admin | — | REQ-UM-022 |
| Letter preview / download | Joining / Relieving | Admin / Officer | Department | REQ-UM-018 |
...
| Biometric device / SDK | Device | Officer login verification | Department only | Must |
...
- **User** — user category (Public / Department), Username, names, mobile, email, KGID (officers), photo, ID proof, biometric refs (officers), active status.
- **SanctionedPost** — post name, abbreviation, acronym, hierarchy level, office, sanctioned strength, occupied count.
- **PostOccupancy** — user (officer), sanctioned post, type (Primary / Secondary), valid from/to, approval letter, status.
...
|------------|-----------|
| Citizen and officer accounts, post occupancy | Permanent for audit |
| Biometric templates (officers) | Per UIDAI policy; re-register every 5 years |
...
| Legacy officers | Map to department users; assign sanctioned posts; re-capture biometrics |
...
| REQ-UM-022 | Department users via designated creator roles | 7.4.1, 8.1 | Add Department User | TC-UM-022 | Draft |
...
| REQ-UM-026 | Officer: Username+OTP+Captcha+Biometrics | 7.6.2, 8.4 | Officer login | TC-UM-026 | Draft |
...
**UAT scope:** Citizen instant registration and Username+OTP+Captcha login; officer creation by designated admin only; sanctioned post mapping and over-capacity block; officer Username+OTP+Captcha+Biometrics login; session role selection; biometric 5-year rule; posts master; designated creator configuration; letters; MIS; audit.

**Go-live gate:** Dual user categories operational; sanctioned posts enforced; passwordless auth live for both categories.
...
| Req ID | Requirement | Priority | Acceptance criteria |
|--------|-------------|----------|---------------------|
| NFR-UM-02 | OTP delivery within 5 seconds for both user categories | Must | 95th percentile ≤ 5 s |
...
| NFR-UM-06 | Biometric data per UIDAI guidelines (department users only) | Must | Security sign-off |

## 16. Risk and Mitigation Strategy

| Risk ID | Risk | Mitigation | Related requirements |
|---------|------|------------|---------------------|
| RS-UM-001 | Unauthorized department user creation | Designated creator role enforcement; audit | REQ-UM-022 |
| RS-UM-002 | Officer assigned to non-sanctioned post | Posts master hard block | REQ-UM-024 |
| RS-UM-003 | Citizen subjected to biometric collection | Separate auth flows; no biometric for citizens | REQ-UM-025 |
| RS-UM-004 | Biometric device failure blocks all officer logins | Admin break-glass with audit (time-limited) | REQ-UM-026, FB-UM-003 |
| RS-UM-005 | Over-capacity post assignment | Sanctioned strength validation | REQ-UM-023, BR-UM-005 |
...
| FB-UM-003 | Biometric device unavailable: time-limited admin break-glass for officers with full audit; citizens unaffected | Must | Officer-only fallback |
...
- Designated admins such as AIGR Admin (officer creation, post mapping).
- All officers (passwordless login with biometrics, session roles).
- Citizens (self-registration and login) — via portal help content.

### 18.2 Training delivery

- Admin training for sanctioned posts and designated creator configuration.
- Officer training for biometric login and role switching.
...
- Communicate biometric enrollment requirement for all officers.
- Pilot with one division before statewide rollout.

### 18.4 Post-Go-Live support

- Hypercare for 30 days; separate helpdesk scripts for citizen vs officer login issues.
...
|--------|----------|------|----------|--------|
| DEC-UM-001 | KAVERI 3.0 — two user categories: Public (Citizen) and Department (Officer) | 2026-08-28 | PO (to confirm) | §7.1 |
| DEC-UM-002 | Instant registration for citizens only; officers via designated admin | 2026-08-28 | PO (to confirm) | REQ-UM-001, REQ-UM-022 |
| DEC-UM-003 | Officers mapped to sanctioned posts only | 2026-08-28 | Domain Expert (to confirm) | REQ-UM-023, REQ-UM-024 |
| DEC-UM-004 | Split auth: citizen Username+OTP+Captcha; officer +Biometrics | 2026-08-28 | Security (to confirm) | REQ-UM-025, REQ-UM-026 |
