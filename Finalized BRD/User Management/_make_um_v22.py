# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v2.2.docx — list Citizen and Other Department roles in Role Master."""
...
DST = BASE / "BRD_User_Management_v2.2.docx"
...
def build() -> Document:
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "Citizen and Role Category = Other Department. Application Admin may add, edit, or "
                "deactivate roles within each Role Category.",
                "High",
            ),
    ...
    sub4 = insert_heading_after(
        sanc_tbl, "6.5.4 Role Master — Citizen and Other Department Roles", "Heading 3", parent=rbac_parent
    )
    note4 = insert_paragraph_after(
        sub4,
        "Citizen and Other Department roles are maintained in the same Role Master (not separate masters), "
        "with Role Category = Citizen or Other Department respectively. Seed roles are listed below. "
        "Application Admin may add further roles under the same Role Categories. Other Department users "
        "are assigned exactly one role with Role Category = Other Department. Primary and Secondary role "
        "distinction does not apply to Other Department users or Citizens. Sanctioned posts do not apply "
        "to these categories.",
        "Normal",
    )
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v2.2"
