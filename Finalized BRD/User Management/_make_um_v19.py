# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v1.9.docx — v1.8 plus Other Dept roles, workflows, Primary/Secondary."""
...
DST = BASE / "BRD_User_Management_v1.9.docx"
...
def build() -> Document:
    ...
    replace_paragraph_text(
        ...
        "OTP-based authentication (no password management), DSR and Other Department role catalogues, "
        "sanctioned posts with approved strength per role for each office, Primary and Secondary role "
        "assignment, and dedicated user-creation workflows. It serves as the agreed basis for design, "
    ...
    replace_paragraph_text(
        ...
        "Other Department users are assigned module access roles from a separate role catalogue. "
        "During user creation, administrators shall designate one Primary Role and may add multiple "
        "Secondary Roles. Password management is explicitly out of scope.",
    )
    ...
    scope_in = {
        ...
        19: "Role-based access control (RBAC) with separate DSR role catalogue and Other Department role catalogue",
        20: "Sanctioned posts master capturing the number of sanctioned posts (approved strength) for each role at each office",
        21: "Primary Role and Secondary Role assignment during DSR and Other Department user creation",
    ...
    for p in doc.paragraphs:
        if p.text.strip() == "Enforce role-based access control to protect sensitive data and functionality.":
            insert_paragraph_after(
                p,
                "Support Primary and Secondary role assignment with mandatory Primary Role at user creation.",
                "List Paragraph",
            )
            break
    ...
    replace_table_rows(
        ...
        ["ID", "Requirement", "Priority"],
        [
            ("FR-16", "The system shall support RBAC with DSR roles mapped to the division and role catalogue (Section 6.5.1) and Other Department roles mapped to the module access role catalogue (Section 6.5.4).", "High"),
            (
                "FR-17",
                "The system shall allow administrators to assign roles to Department users during user creation: "
                "one mandatory Primary Role (mapped to a vacant sanctioned post — role + office) and one or more "
                "optional Secondary Roles from the DSR role catalogue.",
                "High",
            ),
            (
                "FR-28",
                "The system shall maintain a separate Other Department role catalogue (module access roles) "
                "for officers/staff from government departments other than DSR. Other Department users shall "
                "be assigned roles from this catalogue only — not from DSR sanctioned posts.",
                "High",
            ),
            (
                "FR-29",
                "The system shall allow administrators to assign one mandatory Primary Role and one or more "
                "Secondary Roles from the Other Department role catalogue during Other Department user creation.",
                "High",
            ),
            (
                "FR-30",
                "A Primary Role must be selected for each DSR and Other Department user at account creation. "
                "The Primary Role cannot have an end date and represents the user's substantive assignment.",
                "High",
            ),
            (
                "FR-31",
                "The system shall allow administrators to assign multiple Secondary Roles to a user. "
                "Each Secondary Role must have a mandatory end date; the system shall automatically "
                "revoke Secondary Role access on expiry.",
                "High",
            ),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Secondary role not expired on end date", "Medium", "Scheduled job to auto-revoke expired Secondary Roles; audit alert."),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Other Department role", "A module access role from the Other Department role catalogue, assigned to users from government departments other than DSR"),
            ("Module access role", "A role granting access to specific Kaveri modules, scoped to an Other Department user's parent department"),
            ("Primary Role", "The mandatory substantive role assigned at user creation; cannot have an end date"),
            ("Secondary Role", "An additional role assigned to a user; must have a mandatory end date and auto-expires"),
    ...
    sub1 = insert_heading_after(
        rbac_fr_tbl_el, "6.5.1 DSR Division and Role Catalogue", "Heading 3", parent=rbac_parent
    )
    ...
    cat_tbl = insert_table_after(
        ...
        [
            ("Public users (Citizens)", "Citizens accessing Kaveri portal services", "Username + OTP + Captcha"),
            ("Department users (DSR Officers)", "Officers and staff of DSR", "Username + OTP + Captcha + Biometrics"),
            (
    ...
    sub4 = insert_heading_after(
        sanc_tbl, "6.5.4 Other Department Role Catalogue", "Heading 3", parent=rbac_parent
    )
    note4 = insert_paragraph_after(
        sub4,
        "Other Department users (officers/staff from government departments other than DSR) shall be "
        "assigned roles from a separate Other Department role catalogue. These module access roles "
        "grant access to specific Kaveri modules and are not tied to DSR sanctioned posts or offices.",
        "Normal",
    )
    other_tbl = insert_table_after(
        note4,
        ["Parent Department", "Module Access Roles (examples)"],
        [
            ("Revenue Department", "Revenue Verification Officer, Bhoomi Cross-check User"),
            ("Treasury / Finance", "Treasury Payment Verifier, Khajane Reconciliation User"),
            ("Police", "Police Enquiry Officer, FIR Verification User"),
            ("Urban Local Body (ULB)", "ULB Document Verifier"),
            ("General (cross-department)", "Read-only MIS Viewer, Document Upload User"),
        ],
    )

    sub5 = insert_heading_after(
        other_tbl, "6.5.5 Primary and Secondary Role Assignment", "Heading 3", parent=rbac_parent
    )
    primary_note = insert_paragraph_after(
        sub5,
        "When roles are assigned to DSR and Other Department users, the system shall enforce the following:",
        "Normal",
    )
    primary_tbl = insert_table_after(
        ...
        [
            (
                "Primary Role",
                "Yes — exactly one per user",
                "Not permitted",
                "DSR Officers; Other Department users",
                "Substantive assignment; for DSR officers mapped to a vacant sanctioned post",
            ),
            (
                "Secondary Role",
                "No — zero or more",
    ...
    wf_intro = insert_paragraph_after(
        ...
        "Account creation cannot be completed without selecting a Primary Role.",
        "Normal",
    )
    ...
    dsr_tbl = insert_table_after(
        ...
        [
            ...
            ("3", "Select Primary Role — choose vacant sanctioned post (role + office) from posts master", "Admin", "Mandatory; FR-30, FR-26"),
            ("4", "Optionally add Secondary Role(s) from DSR role catalogue", "Admin", "Each requires end date; FR-31"),
    ...
    insert_paragraph_after(
        other_sub,
        "Other Department users are not mapped to DSR sanctioned posts. Roles are selected from the "
        "Other Department role catalogue (Section 6.5.4).",
        "Normal",
    )
    insert_table_after(
        other_sub,
        ...
        [
            ...
            ("2", "Enter user particulars (name, Username, mobile, email, photo, ID proof)", "Admin", ""),
            ("3", "Enter parent department and designation", "Admin", "e.g. Revenue, Treasury, Police"),
            ("4", "Select Primary Role from Other Department role catalogue", "Admin", "Mandatory; FR-30, FR-28"),
            ("5", "Optionally add Secondary Role(s) from Other Department role catalogue", "Admin", "Each requires end date; FR-31"),
            ("6", "Upload authorisation letter / NOC from parent department", "Admin", "Should"),
            ("7", "Capture biometrics", "Admin / User", "Mandatory (FR-07)"),
            ("8", "Review role summary (Primary + Secondary list) and confirm", "Admin", ""),
            ("9", "Save — account active with module access per assigned roles", "System", "Blocked if Primary Role not selected"),
        ],
    )

    core = doc.core_properties
    core.title = "BRD — User Management Module (KAVERI 3.0) v1.9"
