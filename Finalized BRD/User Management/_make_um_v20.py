# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v2.0.docx — Other Dept: single role + optional end-date deactivation."""
...
DST = BASE / "BRD_User_Management_v2.0.docx"
...
def build() -> Document:
    ...
    replace_paragraph_text(
        ...
        "assignment for DSR Officers, single-role assignment for Other Department users with optional "
        "account end date, and dedicated user-creation workflows. It serves as the agreed basis for "
        "design, development, testing, and sign-off.",
    )
    replace_paragraph_text(
        doc.paragraphs[14],
        ...
        "exactly one role from a separate module-access catalogue (no Primary/Secondary distinction) "
        "and may have an optional account end date that deactivates the user when reached. "
        "Password management is explicitly out of scope.",
    )
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "role + office) and one or more optional Secondary Roles from the DSR role catalogue.",
                "High",
            ),
            (
                "FR-28",
            ...
            (
                ...
                "exactly one role from the Other Department role catalogue. Primary and Secondary role "
                "distinction does not apply to Other Department users.",
                "High",
            ),
            (
                "FR-30",
                ...
                "Primary Role does not apply to Other Department users.",
                "High",
            ),
            (
                "FR-31",
                ...
                "revoke Secondary Role access on expiry. Secondary Roles do not apply to Other Department users.",
                "High",
            ),
            (
                "FR-33",
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Other Department role", "A module access role from the Other Department role catalogue; exactly one role is assigned per Other Department user"),
            ...
            ("Primary Role", "The mandatory substantive role assigned to a DSR Officer at user creation; cannot have an end date; does not apply to Other Department users"),
            ("Secondary Role", "An additional role assigned to a DSR Officer; must have a mandatory end date and auto-expires; does not apply to Other Department users"),
    ...
    note4 = insert_paragraph_after(
        ...
        "assigned exactly one role from a separate Other Department role catalogue. These module access "
        "roles grant access to specific Kaveri modules and are not tied to DSR sanctioned posts or offices. "
    ...
    primary_note = insert_paragraph_after(
        ...
        "Other Department users are assigned exactly one role (see Section 6.5.4 and Section 6.6.2).",
        "Normal",
    )
    primary_tbl = insert_table_after(
        ...
        [
            ...
            (
                ...
                "Single module access role; no Primary/Secondary split",
            ),
        ],
    )
    ...
    other_note = insert_paragraph_after(
        other_sub,
        "Other Department users are not mapped to DSR sanctioned posts. Exactly one role is selected "
        "from the Other Department role catalogue (Section 6.5.4). There is no Primary or Secondary role. "
        "An optional End Date may be entered; if entered, the system shall deactivate the user on that date.",
        "Normal",
    )
    insert_table_after(
        other_note,
        ...
        [
            ...
            ("4", "Assign exactly one role from Other Department role catalogue", "Admin", "Mandatory; FR-29, FR-28 — no Primary/Secondary"),
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v2.0"
