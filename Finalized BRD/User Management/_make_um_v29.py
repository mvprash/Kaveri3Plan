# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v2.9.docx — Posts Master, Post–Role map, Sanctioned Posts ref Posts."""
...
DST = BASE / "BRD_User_Management_v2.9.docx"
...
def build() -> Document:
    ...
    replace_paragraph_text(
        ...
        "(Post + Office + strength), Primary and Secondary post assignment "
    ...
    replace_paragraph_text(
        ...
        "sanctioned posts (one Primary plus optional additional posts). Other Department users are assigned "
    ...
    scope_in = {
        ...
        22: "Posts Master (separate), Post–Role mapping (one post → many roles), Sanctioned Posts Master referencing Posts Master; DSR Officers may hold multiple vacant sanctioned posts",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                "FR-17",
                "The system shall allow administrators to assign DSR Officers to vacant sanctioned posts "
                "during user creation: one mandatory Primary post occupancy and zero or more additional "
                "vacant sanctioned posts. Effective roles for the user shall be the union of all roles "
                "mapped to those posts via Post–Role mapping (one post may map to multiple roles).",
                "High",
            ),
            (
                "FR-28",
            ...
            (
                "FR-30",
                "A Primary sanctioned post must be selected for each DSR Officer at account creation. "
                "The Primary post occupancy cannot have an end date and represents the officer's "
                "substantive post. Primary post assignment does not apply to Other Department users or Citizens.",
                "High",
            ),
            (
                "FR-31",
                "The system shall allow administrators to assign multiple additional vacant sanctioned "
                "posts to DSR Officers (Secondary / additional charge). Each additional post occupancy "
                "must have a mandatory end date; on expiry the system shall free occupancy and remove "
                "roles that are no longer granted by any remaining post via Post–Role mapping.",
                "High",
            ),
            (
                "FR-45",
            ...
            (
                ...
                "(e.g. Sub-Registrar, FDA, SDA, DEO, DRO, HQA). Application Admin shall add, edit, "
                "enable/disable, and update posts. Posts Master is distinct from Role Master.",
                "High",
            ),
            (
                "FR-47",
                "The system shall maintain a Post–Role mapping table. One post may be mapped to one or "
                "more roles from the Role Master (Role Category = DSR). Application Admin shall add, "
                "edit, and remove Post–Role mappings with audit trail.",
                "High",
            ),
            (
                "FR-48",
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Primary post", "The mandatory substantive sanctioned post occupancy for a DSR Officer; cannot have an end date"),
            ("Secondary / additional post", "An additional vacant sanctioned post occupancy; mandatory end date; auto-expires"),
            ("Post occupancy", "Assignment of a DSR user to a sanctioned post; effective roles = union of Post–Role mappings for all active posts"),
    ...
    sub3 = insert_heading_after(
        cat_tbl, "6.5.3 Posts Master, Post–Role Mapping, and Sanctioned Posts", "Heading 3", parent=rbac_parent
    )
    note3 = insert_paragraph_after(
        sub3,
        "Posts Master is separate from Role Master. Sanctioned Posts Master references Posts Master "
        "(Post + Office + strength). Post–Role mapping links each Post to one or more Roles "
        "(one post may have multiple roles). DSR Officers are assigned vacant sanctioned posts; "
        "effective roles are the union of roles mapped to those posts. Over-capacity assignment is blocked "
        "(FR-45, FR-46, FR-47, FR-48).",
        "Normal",
    )
    posts_intro = insert_paragraph_after(note3, "Posts Master — seed rows (Application Admin maintains):", "Normal")
    posts_tbl = insert_table_after(
        posts_intro,
        ["Post Code", "Post Name", "Description", "Status"],
        [
            ("POST-IGR", "Inspector General of Registration", "IGR / Commissioner of Stamps", "Active"),
            ("POST-DIGR", "Deputy Inspector General of Registration", "DIGR (by division)", "Active"),
            ("POST-AIGR", "Assistant Inspector General of Registration", "AIGR (by function)", "Active"),
            ("POST-DRO", "District Registrar", "District Registrar / DRO", "Active"),
            ("POST-SR", "Sub-Registrar", "Sub-Registrar office head / signing post", "Active"),
            ("POST-HQA", "Head Quarter Assistant", "HQA", "Active"),
            ("POST-FDA", "First Division Assistant", "FDA", "Active"),
            ("POST-SDA", "Second Division Assistant", "SDA", "Active"),
            ("POST-DEO", "Data Entry Operator", "DEO — SRO operational post", "Active"),
            ("POST-ACCT-SUP", "Accountant Superintendent", "Accountant Superintendent (Admin)", "Active"),
            ("POST-TYPIST", "Typist", "Typist", "Active"),
            ("POST-SI", "Statistical Inspector", "Statistical Inspector", "Active"),
        ],
    )
    map_pr_intro = insert_heading_after(
        posts_tbl,
        "Post–Role mapping — example rows (one Post → one or more Roles):",
        "Normal",
        parent=rbac_parent,
    )
    post_role_tbl = insert_table_after(
        map_pr_intro,
        ["Post (Posts Master)", "Mapped Role (Role Master)", "Role Category", "Notes"],
        [
            ("Sub-Registrar", "Sub-Registrar (SR)", "DSR", "Signing / office head privileges"),
            ("Sub-Registrar", "SRO (Admin)", "DSR", "Example: one post mapped to multiple roles"),
            ("FDA", "FDA", "DSR", ""),
            ("SDA", "SDA", "DSR", ""),
            ("DEO", "DEO", "DSR", ""),
            ("DEO", "Document Registration Operator", "DSR", "Example: DEO post grants an additional operational role"),
            ("District Registrar", "DRO", "DSR", ""),
            ("HQA", "HQA", "DSR", ""),
            ("DIGR", "DIGR (Enforcement)", "DSR", "Division-specific role variant"),
            ("AIGR", "AIGR (Admin)", "DSR", ""),
        ],
    )
    ...
    sanc_tbl = insert_table_after(
        sanc_intro,
        ["Office", "Post (from Posts Master)", "Sanctioned Strength", "Occupied", "Vacant"],
        [
            ("SRO Yeshwanthapura", "Sub-Registrar", "1", "1", "0"),
            ("SRO Yeshwanthapura", "FDA", "2", "1", "1"),
            ("SRO Yeshwanthapura", "SDA", "1", "0", "1"),
            ("SRO Yeshwanthapura", "DEO", "2", "1", "1"),
            ("DRO Mysuru", "District Registrar", "1", "1", "0"),
    ...
    note4 = insert_paragraph_after(
        ...
        "Officers. Sanctioned posts reference Posts Master and apply only to DSR establishment posts.",
        "Normal",
    )
    ...
    primary_note = insert_paragraph_after(
        sub5,
        "DSR Officers are assigned vacant sanctioned posts (from Posts Master via Sanctioned Posts Master). "
        "Exactly one Primary post occupancy is mandatory; additional vacant posts may be assigned. "
        "Effective roles = union of all roles mapped to those posts (Post–Role mapping; one post may "
        "map to multiple roles). Other Department users and Citizens use Role Master directly "
        "(see Sections 6.5.1, 6.5.4 and 6.6.2).",
        "Normal",
    )
    primary_tbl = insert_table_after(
        primary_note,
        ["Assignment", "Mandatory?", "Sanctioned post", "End Date", "Roles granted", "Notes"],
        [
            (
                "Primary post",
                "Yes — exactly one per DSR Officer",
                "One vacant Post + Office",
                "Not permitted",
                "All roles mapped to that Post",
                "Substantive occupancy",
            ),
            (
                "Additional post",
                "No — zero or more",
                "Each must be vacant",
                "Mandatory",
                "Union with roles from other active posts",
                "Additional charge; occupancy freed on end date",
            ),
            (
                "Other Department role",
                "Yes — exactly one role",
                "N/A",
                "N/A (account End Date optional)",
                "The single assigned role",
    ...
    wf_intro = insert_paragraph_after(
        ...
        "For DSR Officers, account creation requires one Primary vacant sanctioned post (Posts Master) "
        "and may include additional vacant posts; roles are derived via Post–Role mapping. "
    ...
    dsr_tbl = insert_table_after(
        ...
        [
            ...
            ("3", "Select Primary sanctioned post — vacant Post + Office (Posts Master / Sanctioned Posts)", "Admin", "Mandatory; FR-30, FR-26, FR-48"),
            ("4", "System resolves roles for the Post via Post–Role mapping (one post may yield multiple roles)", "System", "FR-47"),
            ("5", "Optionally add one or more additional vacant sanctioned posts", "Admin", "Each vacant; end date mandatory; FR-31, FR-45"),
            ("6", "Upload approval letter for additional post(s) where applicable", "Admin", "Should"),
            ("7", "Capture biometrics", "Admin / Officer", "Mandatory for DSR users (FR-06)"),
            ("8", "Review post occupancies and effective roles; confirm", "Admin", "Roles = union across all posts"),
            ("9", "Save — account active; occupied count updated per assigned post", "System", "Blocked if Primary missing or any post not vacant"),
        ],
    )
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v2.9"
