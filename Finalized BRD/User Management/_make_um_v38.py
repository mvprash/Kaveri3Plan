# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v3.8.docx — Transfer out / relieving by hierarchy superior; end-of-day deallocation."""
...
DST = BASE / "BRD_User_Management_v3.8.docx"
...
def build() -> Document:
    ...
    scope_in = {
        ...
        22: "Posts Master, Post–Role mapping, Sanctioned Posts; DSR multi-post assignment; Transfer out / relieving by hierarchy superior with Relieving Date & Order; de-allocate after 11:59 PM of relieving date; Other Department one role with optional end date",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "Relieving shall be performed only by the officer's superior as defined by the DSR Officer "
                "Hierarchy Master (FR-43): the actor's session Post must be the immediate parent Post of "
                "the post being relieved (same reporting line). Example: Sub-Registrar (POST-SR) reports "
                "to DIGR (Enforcement) (POST-DIGR-ENF) — only a user acting under DIGR (Enforcement) may "
                "relieve a Sub-Registrar from that post; FDA (Enforcement) (POST-FDA-ENF) reports to "
                "Sub-Registrar — only a user acting under Sub-Registrar may relieve an FDA (Enforcement) "
                "at that office. While relieving, the system shall capture Relieving Date and Relieving "
                "Order (order number / reference; upload of order document where applicable). Relieving "
                "actions shall be audit-logged. Citizens and Other Department users are out of scope for "
                "this process.",
                "High",
            ),
            (
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Unauthorised relieving", "High", "Allow relieve only if actor session Post is immediate hierarchy parent of target post (FR-57)."),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Relieving / Transfer out", "Process by which a hierarchy superior removes a DSR Officer from an assigned post occupancy, capturing Relieving Date and Relieving Order (FR-57); mapping ends after 11:59 PM of that date (FR-58)"),
    ...
    wf_intro = insert_paragraph_after(
        ...
        "Transfer out / relieving of a post is done only by the hierarchy superior (§6.6.3, FR-57–FR-58). "
    ...
    relieve_note = insert_paragraph_after(
        relieve_sub,
        "A DSR Officer may be relieved from currently assigned post occupancy(ies). Relieving may be "
        "done only by the officer's superior as per the DSR Officer Hierarchy Master (§6.5.7): the "
        "actor must be logged in under the immediate parent Post of the post being relieved (FR-57). "
        "The system shall capture Relieving Date and Relieving Order. User–post mapping is removed "
        "after 11:59 PM of the Relieving Date (FR-58).",
        "Normal",
    )
    relieve_ex_intro = insert_paragraph_after(
        relieve_note,
        "Hierarchy examples (Enforcement — SRO chain from §6.5.7):",
        "Normal",
    )
    relieve_ex_tbl = insert_table_after(
        relieve_ex_intro,
        ["Post being relieved", "Reports To (Hierarchy)", "Who may relieve (superior)", "Example"],
        [
            (
                "Sub-Registrar (POST-SR) at Office A",
                "POST-DIGR-ENF",
                "User session Post = DIGR (Enforcement)",
                "DIGR (Enforcement) relieves Sub-Registrar of SRO Yeshwanthapura",
            ),
            (
                "FDA (Enforcement) (POST-FDA-ENF) at Office A",
                "POST-SR",
                "User session Post = Sub-Registrar",
                "Sub-Registrar of Office A relieves FDA (Enforcement) at Office A",
            ),
            (
                "SDA (Enforcement) (POST-SDA-ENF) at Office A",
                "POST-FDA-ENF",
                "User session Post = FDA (Enforcement)",
                "FDA (Enforcement) at Office A relieves SDA (Enforcement) at Office A",
            ),
            (
                "DEO (POST-DEO) at Office A",
                "POST-SR",
                "User session Post = Sub-Registrar",
                "Sub-Registrar of Office A relieves DEO at Office A",
            ),
        ],
    )
    ...
    insert_table_after(
        ...
        ["Step", "Action", "Actor", "Notes"],
        [
            ("1", "Open Transfer Out / Relieving; select officer and post occupancy (Post + Office)", "Superior", "Must hold parent Post in Hierarchy (FR-57)"),
            ("2", "System validates actor session Post is immediate parent of target Post", "System", "Else block; refer Hierarchy Master"),
            ("3", "Enter Relieving Date", "Superior", "Mandatory (FR-57)"),
            ("4", "Enter Relieving Order number / reference; upload order if applicable", "Superior", "Mandatory order reference (FR-57)"),
            ("5", "Confirm relieving", "Superior", "Audit-logged"),
            ("6", "Until end of Relieving Date — occupancy remains active", "System", "Officer may still use post until day closes"),
            ("7", "After 11:59 PM of Relieving Date — de-allocate user–post mapping", "System", "FR-58; occupied count −1; post vacant"),
            ("8", "If no remaining post occupancies — block / limit login until new assignment", "System", "Per policy; audit"),
        ],
    )

    core = doc.core_properties
    core.title = "BRD — User Management Module (KAVERI 3.0) v3.8"
