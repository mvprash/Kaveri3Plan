# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v4.8.docx — concrete OTP/session FRs; complete Post–Role and
Post–Office-Type seeds; Sub Registrar (Computers); MS Building; §2.3/§9 cleanup."""
...
DST = BASE / "BRD_User_Management_v4.8.docx"
...
def build() -> Document:
    ...
    scope_in = {
        ...
        21: "DSR Officer Hierarchy Master (posts), Office Hierarchy Master, and Division Master (enumerated divisions)",
        22: "Post–Office-Type validation on sanctioned posts; Transfer Out / Relieving and Transfer In (office span then immediate-parent post only; future-dated Transfer In may reserve capacity when relieving is already recorded; Transfer/Reporting Order + Joining Date; login from 12:00 AM IST on Joining Date); midnight occupancy refresh job; office span via Office Hierarchy",
    ...
    for p in doc.paragraphs:
        if "[applicable data protection regulation" in p.text:
            replace_paragraph_text(
                p,
                "The module must comply with the Digital Personal Data Protection Act, 2023 (DPDP Act) "
                "and Karnataka state e-Governance and data-protection policies applicable to citizen and "
                "government-user data.",
            )
        if "[database / identity infrastructure]" in p.text:
            replace_paragraph_text(
                p,
                "The solution must integrate with the KAVERI 3.0 platform database, identity and "
                "authentication services, SMS and email gateways, and Aadhaar-compatible biometric "
                "authentication infrastructure deployed for the Department.",
            )
        if "[Product Owner]" in p.text:
            replace_paragraph_text(
                p,
                "User acceptance testing (UAT) has been completed and signed off by Prashanth (Product Owner), "
                "including the scenarios listed in Section 9.",
            )
    ...
    for p in doc.paragraphs:
        if p.text.strip() == "All functional requirements listed in Section 6 have been implemented and pass QA testing.":
            replace_paragraph_text(
                p,
                "All functional requirements listed in Section 6 have been implemented and pass QA testing, "
                "including the UAT scenarios below.",
            )
            u1 = insert_paragraph_after(
                p,
                "UAT — FR-52: A DSR Officer with two active post occupancies must select one post labelled "
                "Role — Post Name — Office Name (Office Code) before home; a single occupancy auto-selects; "
                "claims derive only from the selected assigned post.",
                "List Paragraph",
            )
            u2 = insert_paragraph_after(
                u1,
                "UAT — FR-53: After login at SRO Yeshwanthapura, additional charge lists only wholly unoccupied "
                "subordinate posts at that same office; empty posts at another SRO (e.g. Jayanagar) are not "
                "offered; privileges switch to the additional-charge post only until switch-back (FR-38).",
                "List Paragraph",
            )
            u3 = insert_paragraph_after(
                u2,
                "UAT — FR-58 / FR-68: Relieving with Relieving Date D keeps occupancy through 11:59 PM IST of D; "
                "the occupancy refresh job shortly after midnight IST de-allocates it and updates occupied counts.",
                "List Paragraph",
            )
            u4 = insert_paragraph_after(
                u3,
                "UAT — FR-61 / FR-67: Future-dated Transfer In against a full post is allowed only after relieving "
                "is recorded; reserved occupancy counts toward Occupied immediately; login for that post is "
                "blocked until 12:00 AM IST of Joining Date; the midnight job activates it that day.",
                "List Paragraph",
            )
            insert_paragraph_after(
                u4,
                "UAT — FR-69–FR-76: Login OTP expires after 5 minutes; 6 digits; 3 incorrect entries invalidate "
                "the code; 30-second resend cooldown; 5 failed logins lock Username 15 minutes; idle timeout "
                "15 minutes; session ends at 8 hours; a second login ends the prior session.",
                "List Paragraph",
            )
            break
    ...
    for p in doc.paragraphs:
        if p.text.strip() == "The system shall provide a report of role and permission assignments across all users.":
            ...
            ac_note = insert_paragraph_after(
                rm_note,
                "The system shall provide a single contact-change and recovery report over a selected date "
                "range covering: (a) Citizen lost-mobile / security-question recovery attempts (FR-56) — "
                "Username, questions presented, PIN dispatch outcome, new mobile OTP outcome, "
                "success/failure, actor, and timestamp; (b) administrator-initiated mobile changes for "
                "departmental users (FR-65); and (c) email changes (Citizen self-service or admin) — "
                "showing actor, user, channel changed, timestamp, outcome, and failed attempt counts. "
                "One report; do not build separate overlapping recovery reports.",
                "List Paragraph",
            )
            ...
            occ_note = insert_paragraph_after(
                job_note,
            ...
            ti_note = insert_paragraph_after(
                occ_note,
                "The system shall provide a Transfer Out / Transfer In history report (FR-57–FR-61, "
                "FR-67) over a selected date range showing each relieving and Transfer In event — actor, "
                "officer, Post + Office, order reference, Relieving Date or Joining Date, reserved vs "
                "active status, and audit timestamp.",
                "List Paragraph",
            )
            posting_note = insert_paragraph_after(
                ti_note,
                "The system shall provide an officer posting and service history report showing, for each "
                "DSR Officer, the chronological list of post occupancies (Post + Office), assignment / "
                "Transfer In / relieving / End Date events, and effective date ranges.",
                "List Paragraph",
            )
            break
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "30-Aug-2026",
                "Nandha Kumar",
                "FR-77 Division Master added; FR-78 Post–Office-Type validation on sanctioned posts. "
                "Expanded §8 reporting (Transfer In/Out history, officer posting history, lost-mobile "
                "recovery). Karnataka e-Gov NFRs (Kannada/bilingual UI, GIGW, data residency, DR, "
                "retention). §5.2 / §9 template placeholders resolved. Biometrics remain mandatory "
                "with no fallback (item 15). No migration scope or maker-checker (items 16, 22). "
                "KGID confirmed as Username for departmental users (item 17).",
            ),
            (
                "4.7",
                "30-Aug-2026",
                "Nandha Kumar",
                "§6.5.4 DSR role catalogue uses Division Code (FR-77). FR-46 requires Division Code on "
                "Posts Master. FR-17 wording and FR-52 login-step label aligned. Glossary / risk / "
                "workflow timings state IST. §8 contact-change and recovery report merged (FR-56 + "
                "admin contact changes). FR-53 same-office example retained from v4.6 errata.",
            ),
            (
                "4.8",
                "30-Aug-2026",
                "Nandha Kumar",
                "FR-69–FR-76 PLACEHOLDER text replaced with concrete OTP and session values. "
                "Post–Role mapping and Post–Office-Type Allowed seeds completed for all active Posts. "
                "Post Name Sub Registrar (Computers) aligned. MS Building added as Secretariat office "
                "(ACS). Template SaaS Out-of-Scope lines removed; §9 generic acceptance replaced with "
                "UAT scenarios for FR-52/53/58/61/67/68.",
            ),
        ],
    )
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "category (FR-62, FR-64) and is the governed unique login identifier — official email ID, "
                "and mobile number. Security questions are not captured for this category because no "
                "self-service mobile reset is offered (FR-65).",
                "High",
            ),
            (
                "FR-03",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                "FR-69",
                "The system shall enforce OTP and PIN validity as follows (IST clock): login OTP is valid "
                "for five (5) minutes from dispatch; Citizen registration email OTP, registration mobile OTP "
                "(FR-63), and lost-mobile reset PIN (FR-56) are each valid for ten (10) minutes from "
                "dispatch. On expiry the code is invalid and the user must request a new one.",
                "High",
            ),
            (
                "FR-70",
                "Login OTPs, registration OTPs, and the FR-56 reset PIN shall be six (6) numeric digits.",
                "High",
            ),
            (
                "FR-71",
                "The system shall allow a maximum of three (3) incorrect entries per issued OTP or reset PIN. "
                "After the third failure that code is invalidated and the user must request a new one.",
                "High",
            ),
            (
                "FR-72",
                "The system shall enforce a thirty (30) second cooldown between successive OTP or PIN resend "
                "requests to the same user and channel (mobile or email), and a maximum of three (3) resends "
                "per channel per fifteen (15) minutes.",
                "High",
            ),
            (
                "FR-73",
                "After five (5) failed login or OTP verification attempts for a Username (separate from FR-56 "
                "lost-mobile reset lockout), the system shall lock that Username for fifteen (15) minutes. "
                "Unlock is automatic when the lockout expires, or earlier by an authorised administrator "
                "with reason and audit trail. FR-56 reset-flow lockout: five (5) failed question or PIN "
                "attempts lock that flow for thirty (30) minutes.",
                "High",
            ),
            (
                "FR-74",
                "The system shall terminate a session after fifteen (15) minutes of no user activity "
                "(idle timeout). Re-authentication is then required (OTP, and biometrics for departmental "
                "users). This applies especially to shared SRO counter machines where FR-52/FR-53 fix the "
                "session post for the whole session.",
                "High",
            ),
            (
                "FR-75",
                "The system shall terminate a session after an absolute maximum of eight (8) hours from "
                "login, even if the user is still active. Re-authentication is required to continue.",
                "High",
            ),
            (
                "FR-76",
                "The system shall allow only one active session per Username. A new successful login "
                "terminates any prior session for that Username (last-login-wins) and shall notify the "
                "prior session on next request that it has ended.",
                "High",
            ),
            (
                "FR-52",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "MS Building is the Secretariat office (root); under it is IGR Office (Head Office); "
                "under the Head Office are District Registrar Offices (DRO); under each District Registrar "
                "Office are Sub-Registrar Offices (SRO). "
                "Each Office shall have Office Code, Office Name, Office Type (Secretariat / Head Office / "
                "District Registrar Office / Sub-Registrar Office), and optional Parent Office. Application Admin "
                ...
                "POST-ACS-SEC is sanctioned at MS Building (Secretariat).",
                "High",
            ),
            (
                "FR-60",
            ...
            (
                ...
                "so that from 12:00 AM IST on the Joining Date the post occupancy is available for login "
            ...
            (
                ...
                "the occupancy being replaced (i.e. after 11:59 PM IST of Relieving Date — example: "
            ...
            (
                "FR-77",
                "The system shall maintain a Division Master — an enumerated list of DSR organisational "
                "divisions (Division Code, Division Name, Display Order, Is Active). Posts Master, "
                "Hierarchy Master nodes, and Role Master seed groupings shall reference Division Code "
                "from this master — free-text division labels shall not be used on new or edited records. "
                "Application Admin shall add, edit, enable/disable, and update divisions with audit trail. "
                "Seed divisions are listed in Section 6.5.1.",
                "High",
            ),
            (
                "FR-78",
                "The system shall maintain a Post–Office-Type Allowed mapping defining which Post codes "
                "(from Posts Master) may be sanctioned at which Office Types (from Office Hierarchy "
                "Master — Secretariat, Head Office, District Registrar Office, Sub-Registrar Office). When creating "
                "or updating a Sanctioned Posts row or assigning a post to an officer, the system shall "
                "reject combinations not permitted by this mapping (FR-24, FR-48). Seed mapping for all "
                "active Posts is listed in Section 6.5.3 (e.g. POST-ACS-SEC only at Secretariat / MS Building; "
                "POST-SR, POST-FDA-ENF, POST-SDA-ENF, POST-DEO only at Sub-Registrar Office; "
                "POST-DRO only at District Registrar Office; POST-IGR and Head Office division posts only at "
                "Head Office). Application Admin shall maintain the mapping with audit trail.",
                "High",
            ),
            (
                "FR-46",
                ...
                "Each Posts Master row shall reference a Division Code from the Division Master (FR-77). "
            ...
            (
                ...
                "this BRD are the complete seed mapping for all active Posts; Application Admin may add "
                "further mappings with audit trail. Domain Expert (Prabhakar Naik) shall confirm the "
                "seed list before go-live.",
                "High",
            ),
            (
                "FR-48",
                ...
                "strength. The Post + Office Type combination shall be validated against the "
                "Post–Office-Type Allowed mapping (FR-78). Strength, occupied count, remaining "
            ...
            (
                ...
                "(or other) row in the Role–Module–Function example catalogue. Dual authorisation / "
                "maker-checker is not required for user creation or master maintenance in this module. "
                "DSR and Other Department user creation does not capture security questions (FR-55, FR-65).",
                "High",
            ),
            (
                "FR-42",
            ...
            ("FR-19", "The system shall maintain DSR organizational divisions in the Division Master (FR-77) and DSR roles within the unified Role Master (Role Category = DSR) as defined in Section 6.5.1 and the DSR Officer Hierarchy Master in Section 6.5.7.", "High"),
            (
                "FR-43",
            ...
            (
                ...
                "Sanctioned strength applies only to DSR Posts Master entries. Each row shall satisfy "
                "Post–Office-Type Allowed rules (FR-78).",
                "High",
            ),
            (
                "FR-25",
                ...
                "Invalid Post + Office Type combinations shall be blocked (FR-78).",
                "High",
            ),
            (
                "FR-26",
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Security", "OTP and session policy is defined in FR-69–FR-76: login OTP 5 minutes; registration/recovery OTP and reset PIN 10 minutes; 6 digits; 3 incorrect entries per code; 30-second resend cooldown (max 3 per 15 minutes); 5 failed login attempts lock Username 15 minutes; idle timeout 15 minutes; absolute session 8 hours; one active session per Username."),
            ...
            ("Compliance", "Citizen-facing UI shall support Kannada and English (bilingual) per Karnataka e-Governance norms."),
            ("Compliance", "Citizen-facing UI shall comply with Government of India Guidelines for Indian Government Websites (GIGW) accessibility requirements."),
            ("Compliance", "Personal and authentication data shall reside in India; production hosting shall use MeitY-empanelled cloud / data-centre infrastructure as applicable to KAVERI 3.0."),
            ("Compliance", "The module shall comply with Karnataka e-Governance, MeitY/CERT-In, the DPDP Act 2023, and applicable Karnataka state data-protection norms."),
            ("Performance", "The system shall support a minimum of 500 concurrent authenticated user sessions at launch; peak-load targets shall be validated during performance testing with Kaveri IT Cell."),
            ("Availability", "Disaster recovery shall target RPO ≤ 24 hours and RTO ≤ 4 hours for the User Management module and its database, subject to platform-wide DR architecture."),
            ("Usability", "Supported browsers: latest two stable versions of Google Chrome and Microsoft Edge on Windows; citizen flows shall also work on current Android and iOS mobile browsers."),
            ("Auditability", "Audit logs for security, authentication, access control, Transfer Out / Transfer In, and administrative actions shall be retained for at least seven (7) years unless a longer DSR records-retention rule applies."),
            ("Compliance", "PII retention and purge policy: user PII shall be retained while the account is active and for the audit-retention period thereafter; purge/anonymisation of dormant Citizen accounts shall follow a policy defined by the Department before go-live."),
        ],
    )
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Biometric device unavailability", "Medium", "Biometrics are mandatory on every login for DSR and Other Department users (FR-06, FR-07) with no exception path — offices must maintain working devices; no biometric fallback is in scope."),
            ...
            ("Unmapped post assigned or sanctioned", "High", "FR-47 blocks sanction/assignment without Post–Role mapping; seed mapping covers all active Posts; Domain Expert to confirm before go-live."),
            ("OTP/session policy", "High", "Enforce FR-69–FR-76 as specified (including 15-minute idle timeout on shared SRO counters with FR-52/FR-53)."),
            ...
            ("Joining Date login gate not enforced", "High", "Block login for Transfer In occupancy until 12:00 AM IST of Joining Date (FR-61); FR-68 activates reserved occupancy that day."),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("KGID", "Karnataka Government Insurance Department number — used as the Username (unique login identifier) for DSR Officers and Other Department users (FR-62, FR-64); not a separate field from Username"),
            ("Division Master", "Enumerated master of DSR organisational divisions (Division Code, Division Name); referenced by Posts Master and Hierarchy Master (FR-77)"),
            ("Post–Office-Type Allowed mapping", "Rules defining which Post codes may be sanctioned at which Office Types; invalid combinations blocked (FR-78)"),
            ...
            ("Office Hierarchy Master", "Tree of DSR offices: MS Building (Secretariat, root) → IGR Head Office → District Registrar Offices → Sub-Registrar Offices (FR-59)"),
            ("MS Building", "Secretariat office (Office Code OFF-MS-BLDG); Office Hierarchy root; office of POST-ACS-SEC (ACS / Secretary)"),
            ("Secretariat (Office Type)", "Office Type for MS Building, distinct from Head Office (IGR Office) (FR-59, FR-78)"),
            ...
            ("Relieving / Transfer out", "Process by which a superior removes a DSR Officer from a post occupancy within the superior's office span and immediate-parent posts only, capturing Relieving Date and Relieving Order (FR-57–FR-59); mapping ends after 11:59 PM IST of that date via the occupancy refresh job (FR-58, FR-68)"),
            ("Relieving Date", "Calendar date captured during relieving; user–post mapping is removed after 11:59 PM IST of this date by the occupancy refresh job (FR-68)"),
            ("Relieving Order", "Order number / reference (and optional uploaded order document) recorded when relieving an officer"),
            ("Transfer In", "Process by which a superior assigns a DSR Officer to a post within office span and immediate-parent posts only, capturing Transfer/Reporting Order and Joining Date (FR-60, FR-66(a)); when the post is full, allowed only if relieving is already recorded and capacity is reserved (FR-67); login from 12:00 AM IST on Joining Date (FR-61)"),
            ...
            ("Joining Date", "Calendar date captured during Transfer In; login for that post occupancy is allowed only from 12:00 AM IST of this date onwards; the occupancy refresh job activates reserved occupancies on this date (FR-61, FR-68)"),
    ...
    div_master_sub = insert_heading_after(
        cat_diff_tbl,
        "Division Master (FR-77):",
        "Normal",
        parent=rbac_parent,
    )
    div_master_note = insert_paragraph_after(
        div_master_sub,
        "DSR divisions are governed by the Division Master — not free text. Posts Master, Hierarchy "
        "Master, and role catalogues reference Division Code from this master.",
        "Normal",
    )
    div_master_tbl = insert_table_after(
        div_master_note,
        ["Division Code", "Division Name", "Display Order", "Status"],
        [
            ("DIV-SECRETARIAT", "Secretariat", "1", "Active"),
            ("DIV-TOP-MGMT", "Top Management", "2", "Active"),
            ("DIV-ADMIN", "Admin, Law & Computers", "3", "Active"),
            ("DIV-VIGILANCE", "Vigilance", "4", "Active"),
            ("DIV-COMPUTERS", "Computers", "5", "Active"),
            ("DIV-ENFORCEMENT", "Enforcement", "6", "Active"),
            ("DIV-INT-AUDIT", "Intelligence & Audit", "7", "Active"),
            ("DIV-CVC", "DIGR CVC", "8", "Active"),
        ],
    )
    ...
    div_tbl = insert_table_after(
        div_intro,
        ["Division (FR-77)", "Roles (Role Category = DSR) — unique names"],
        [
            ("DIV-SECRETARIAT", "ACS / Principal Secretary / Secretary"),
            ("DIV-TOP-MGMT", "IGR"),
            (
                "DIV-ADMIN",
            ...
            ("DIV-VIGILANCE", "DIGR (Vigilance), Law Officer"),
            (
                "DIV-COMPUTERS",
            ...
            ("DIV-ENFORCEMENT", "DIGR (Enforcement), DRO, HQA (Enforcement), Sub-Registrar (SR), FDA (Enforcement), SDA (Enforcement), DEO"),
            (
                "DIV-INT-AUDIT",
                "DIGR (Intelligence), AIGR (Audit), HQA (Audit), Superintendent (Audit), FDA (Audit), SDA (Audit), Typist (Audit)",
            ),
            ("DIV-CVC", "DIGR CVC, JD Town Planning"),
        ],
    )
    ...
    note2 = insert_paragraph_after(
        ...
        "charge (FR-54). OTP and session rules are defined in FR-69–FR-76 (login OTP 5 minutes, idle "
        "timeout 15 minutes, one session per Username).",
        "Normal",
    )
    ...
    login_post_tbl = insert_table_after(
        ...
        [
            ...
            ("3a", "If exactly one active post — auto-select; continue to home", "System", "No selection UI; exclude posts before Joining Date 12:00 AM IST (FR-61)"),
            ...
            ("4", "Display each option as Role — Post Name — Office Name (Office Code)", "UI", "Same unified label as FR-52 / FR-53 / FR-54; Office details mandatory on each row"),
    ...
    switch_ex_intro = insert_heading_after(
        switch_tbl,
        "Illustrative example (SR logged in at SRO Yeshwanthapura — additional charge after login). "
        "Only wholly unoccupied subordinate posts at the same Office (OFF-SRO-YESH) are offered (FR-53); "
        "posts at other offices are never offered:",
        "Normal",
        parent=rbac_parent,
    )
    switch_ex_tbl = insert_table_after(
        ...
        [
            ...
            (
                ...
                "2",
                "0",
                "Yes — nobody is posted against it at this office",
                "DEO — Data Entry Operator — SRO Yeshwanthapura (OFF-SRO-YESH)",
            ),
            (
                "SRO Jayanagar (different office)",
                "→ FDA (Enforcement) / SDA (Enforcement)",
                "2 / 1",
                "0 / 0",
                "No — never offered; additional charge is same Office only (FR-53). "
                "Empty posts at Jayanagar are occupancy facts for that office, not picker options here.",
                "Not listed",
            ),
            (
    ...
    note3 = insert_paragraph_after(
        ...
        "Sanctioned posts must satisfy Post–Office-Type Allowed rules (FR-78).",
        "Normal",
    )
    posts_intro = insert_paragraph_after(note3, "Posts Master — seed rows (division-specific; Division Code from Division Master — FR-77):", "Normal")
    posts_tbl = insert_table_after(
        posts_intro,
        ["Post Code", "Post Name", "Division Code (FR-77)", "Status"],
        [
            ("POST-ACS-SEC", "Additional Chief Secretary / Principal Secretary / Secretary", "DIV-SECRETARIAT", "Active"),
            ("POST-IGR", "Inspector General of Registration & Commissioner of Stamps", "DIV-TOP-MGMT", "Active"),
            ("POST-DIGR-ADMIN", "DIGR (Admin, Law & Computers)", "DIV-ADMIN", "Active"),
            ("POST-AIGR-ADMIN", "AIGR (Admin)", "DIV-ADMIN", "Active"),
            ("POST-HQA-ADMIN", "HQA (Admin)", "DIV-ADMIN", "Active"),
            ("POST-SR-ADMIN", "Sub Registrar (Admin)", "DIV-ADMIN", "Active"),
            ("POST-ACCT-SUP", "Accountant Superintendent (Admin)", "DIV-ADMIN", "Active"),
            ("POST-FDA-ADMIN", "FDA (Admin)", "DIV-ADMIN", "Active"),
            ("POST-SDA-ADMIN", "SDA (Admin)", "DIV-ADMIN", "Active"),
            ("POST-TYPIST-ADMIN", "Typist (Admin)", "DIV-ADMIN", "Active"),
            ("POST-HQA-RTI", "HQA (RTI)", "DIV-ADMIN", "Active"),
            ("POST-FDA-RTI", "FDA (RTI)", "DIV-ADMIN", "Active"),
            ("POST-SDA-RTI", "SDA (RTI)", "DIV-ADMIN", "Active"),
            ("POST-SI", "Statistical Inspector", "DIV-ADMIN", "Active"),
            ("POST-DIGR-VIG", "DIGR (Vigilance)", "DIV-VIGILANCE", "Active"),
            ("POST-LAW-OFF", "Law Officer", "DIV-VIGILANCE", "Active"),
            ("POST-AIGR-COMP", "AIGR (Computers)", "DIV-COMPUTERS", "Active"),
            ("POST-SI-INT", "System Integrator", "DIV-COMPUTERS", "Active"),
            ("POST-APP-DEV", "Application Developer", "DIV-COMPUTERS", "Active"),
            ("POST-PMU", "PMU", "DIV-COMPUTERS", "Active"),
            ("POST-HQA-COMP", "HQA / Project Manager (Comp)", "DIV-COMPUTERS", "Active"),
            ("POST-SR-COMP", "Sub Registrar (Computers)", "DIV-COMPUTERS", "Active"),
            ("POST-FDA-COMP", "FDA (Computers)", "DIV-COMPUTERS", "Active"),
            ("POST-SDA-COMP", "SDA (Computers)", "DIV-COMPUTERS", "Active"),
            ("POST-DIGR-ENF", "DIGR (Enforcement)", "DIV-ENFORCEMENT", "Active"),
            ("POST-DRO", "District Registrar", "DIV-ENFORCEMENT", "Active"),
            ("POST-HQA-ENF", "HQA (Enforcement)", "DIV-ENFORCEMENT", "Active"),
            ("POST-SR", "Sub-Registrar", "DIV-ENFORCEMENT", "Active"),
            ("POST-FDA-ENF", "FDA (Enforcement)", "DIV-ENFORCEMENT", "Active"),
            ("POST-SDA-ENF", "SDA (Enforcement)", "DIV-ENFORCEMENT", "Active"),
            ("POST-DEO", "Data Entry Operator", "DIV-ENFORCEMENT", "Active"),
            ("POST-DIGR-INT", "DIGR (Intelligence)", "DIV-INT-AUDIT", "Active"),
            ("POST-AIGR-AUDIT", "AIGR (Audit)", "DIV-INT-AUDIT", "Active"),
            ("POST-HQA-AUDIT", "HQA (Audit)", "DIV-INT-AUDIT", "Active"),
            ("POST-SUP-AUDIT", "Superintendent (Audit)", "DIV-INT-AUDIT", "Active"),
            ("POST-FDA-AUDIT", "FDA (Audit)", "DIV-INT-AUDIT", "Active"),
            ("POST-SDA-AUDIT", "SDA (Audit)", "DIV-INT-AUDIT", "Active"),
            ("POST-TYPIST-AUDIT", "Typist (Audit)", "DIV-INT-AUDIT", "Active"),
            ("POST-DIGR-CVC", "DIGR CVC", "DIV-CVC", "Active"),
            ("POST-JD-TP", "JD Town Planning", "DIV-CVC", "Active"),
        ],
    )
    map_pr_intro = insert_heading_after(
        posts_tbl,
        "Post–Role mapping — seed rows for all active Posts (division-aligned; avoid cross-division "
        "over-provisioning). Post Name must match Posts Master character-for-character (FR-50). "
        "FR-47 blocks sanction or assignment of any Post with no mapping row:",
        "Normal",
        parent=rbac_parent,
    )
    post_role_tbl = insert_table_after(
        ...
        [
            ("POST-ACS-SEC", "Additional Chief Secretary / Principal Secretary / Secretary", "ACS / Principal Secretary / Secretary", "1:1 — hierarchy root; sanctioned at MS Building"),
            ...
            ("POST-HQA-ADMIN", "HQA (Admin)", "HQA (Admin)", "1:1"),
            ("POST-SR-ADMIN", "Sub Registrar (Admin)", "Sub Registrar (Admin)", "1:1"),
            ("POST-ACCT-SUP", "Accountant Superintendent (Admin)", "Accountant Superintendent (Admin)", "1:1"),
            ("POST-FDA-ADMIN", "FDA (Admin)", "FDA (Admin)", "Unique role — not shared FDA"),
            ("POST-SDA-ADMIN", "SDA (Admin)", "SDA (Admin)", "Unique role"),
            ("POST-TYPIST-ADMIN", "Typist (Admin)", "Typist (Admin)", "1:1"),
            ("POST-HQA-RTI", "HQA (RTI)", "HQA (RTI)", "1:1"),
            ("POST-FDA-RTI", "FDA (RTI)", "FDA (RTI)", "1:1"),
            ("POST-SDA-RTI", "SDA (RTI)", "SDA (RTI)", "1:1"),
            ("POST-SI", "Statistical Inspector", "Statistical Inspector", "1:1"),
            ("POST-DIGR-VIG", "DIGR (Vigilance)", "DIGR (Vigilance)", "1:1 — division-specific"),
            ("POST-LAW-OFF", "Law Officer", "Law Officer", "1:1"),
            ("POST-AIGR-COMP", "AIGR (Computers)", "AIGR (Computers)", "1:1"),
            ("POST-SI-INT", "System Integrator", "System Integrator", "1:1"),
            ("POST-APP-DEV", "Application Developer", "Application Developer", "1:1"),
            ("POST-PMU", "PMU", "PMU", "1:1"),
            ("POST-HQA-COMP", "HQA / Project Manager (Comp)", "HQA / Project Manager (Comp)", "1:1"),
            ("POST-SR-COMP", "Sub Registrar (Computers)", "Sub Registrar (Computers)", "1:1 — Post Name matches Role"),
            ("POST-FDA-COMP", "FDA (Computers)", "FDA (Computers)", "1:1"),
            ("POST-SDA-COMP", "SDA (Computers)", "SDA (Computers)", "1:1"),
            ("POST-DIGR-ENF", "DIGR (Enforcement)", "DIGR (Enforcement)", "1:1 — division-specific"),
            ("POST-DRO", "District Registrar", "DRO", "1:1"),
            ("POST-HQA-ENF", "HQA (Enforcement)", "HQA (Enforcement)", "1:1"),
            ...
            ("POST-HQA-AUDIT", "HQA (Audit)", "HQA (Audit)", "1:1"),
            ("POST-SUP-AUDIT", "Superintendent (Audit)", "Superintendent (Audit)", "1:1"),
            ("POST-FDA-AUDIT", "FDA (Audit)", "FDA (Audit)", "Unique role"),
            ("POST-SDA-AUDIT", "SDA (Audit)", "SDA (Audit)", "1:1"),
            ("POST-TYPIST-AUDIT", "Typist (Audit)", "Typist (Audit)", "1:1"),
            ("POST-DIGR-CVC", "DIGR CVC", "DIGR CVC", "1:1"),
            ("POST-JD-TP", "JD Town Planning", "JD Town Planning", "1:1"),
        ],
    )
    ...
    sanc_tbl = insert_table_after(
        ...
        [
            ("OFF-MS-BLDG", "MS Building", "POST-ACS-SEC", "Additional Chief Secretary / Principal Secretary / Secretary", "1", "1", "0", "No — full", "No"),
    ...
    post_office_intro = insert_heading_after(
        sanc_tbl,
        "Post–Office-Type Allowed mapping — seed rows for all active Posts (FR-78):",
        "Normal",
        parent=rbac_parent,
    )
    post_office_tbl = insert_table_after(
        post_office_intro,
        ["Post Code", "Allowed Office Type(s)", "Notes"],
        [
            ("POST-ACS-SEC", "Secretariat", "ACS / Secretary at MS Building only"),
            ("POST-IGR", "Head Office", "IGR at IGR Office"),
            ("POST-DIGR-ADMIN", "Head Office", "Admin DIGR"),
            ("POST-AIGR-ADMIN", "Head Office", "Admin AIGR"),
            ("POST-HQA-ADMIN", "Head Office", "Admin HQ"),
            ("POST-SR-ADMIN", "Head Office", "Admin Sub Registrar"),
            ("POST-ACCT-SUP", "Head Office", "Admin accounts"),
            ("POST-FDA-ADMIN", "Head Office", "Admin FDA"),
            ("POST-SDA-ADMIN", "Head Office", "Admin SDA"),
            ("POST-TYPIST-ADMIN", "Head Office", "Admin Typist"),
            ("POST-HQA-RTI", "Head Office", "RTI HQ"),
            ("POST-FDA-RTI", "Head Office", "RTI FDA"),
            ("POST-SDA-RTI", "Head Office", "RTI SDA"),
            ("POST-SI", "Head Office", "Statistical Inspector"),
            ("POST-DIGR-VIG", "Head Office", "Vigilance DIGR"),
            ("POST-LAW-OFF", "Head Office", "Law Officer"),
            ("POST-AIGR-COMP", "Head Office", "Computers AIGR"),
            ("POST-SI-INT", "Head Office", "System Integrator"),
            ("POST-APP-DEV", "Head Office", "Application Developer"),
            ("POST-PMU", "Head Office", "PMU"),
            ("POST-HQA-COMP", "Head Office", "Computers HQ / PM"),
            ("POST-SR-COMP", "Head Office", "Sub Registrar (Computers)"),
            ("POST-FDA-COMP", "Head Office", "Computers FDA"),
            ("POST-SDA-COMP", "Head Office", "Computers SDA"),
            ("POST-DIGR-ENF", "Head Office", "Enforcement DIGR"),
            ("POST-DRO", "District Registrar Office", "District Registrar only at DRO"),
            ("POST-HQA-ENF", "District Registrar Office, Head Office", "Enforcement HQA at DRO or Head Office"),
            ("POST-SR", "Sub-Registrar Office", "Field Sub-Registrar"),
            ("POST-FDA-ENF", "Sub-Registrar Office", "Enforcement FDA at SRO"),
            ("POST-SDA-ENF", "Sub-Registrar Office", "Enforcement SDA at SRO"),
            ("POST-DEO", "Sub-Registrar Office", "Data Entry Operator at SRO"),
            ("POST-DIGR-INT", "Head Office", "Intelligence DIGR"),
            ("POST-AIGR-AUDIT", "Head Office", "Audit AIGR"),
            ("POST-HQA-AUDIT", "Head Office", "Audit HQ"),
            ("POST-SUP-AUDIT", "Head Office", "Superintendent (Audit)"),
            ("POST-FDA-AUDIT", "Head Office", "Audit FDA"),
            ("POST-SDA-AUDIT", "Head Office", "Audit SDA"),
            ("POST-TYPIST-AUDIT", "Head Office", "Audit Typist"),
            ("POST-DIGR-CVC", "Head Office", "DIGR CVC"),
            ("POST-JD-TP", "Head Office", "JD Town Planning"),
        ],
    )
    ...
    admin_posts_tbl = insert_table_after(
        ...
        [
            ("Posts Master", "Add, edit, enable/disable posts; Division Code from Division Master (FR-77)", "FR-46, FR-77"),
            ("Division Master", "Add, edit, enable/disable divisions", "FR-77"),
            ("Post–Role mapping", "Map one Post to one or more Roles; block sanction/assignment if unmapped (FR-47)", "FR-47"),
            ("Post–Office-Type Allowed mapping", "Define which Post codes may be sanctioned at which Office Types", "FR-78"),
            ("Sanctioned Posts Master", "Set strength per Post per Office (Office Code); validate Office Type; view occupied count (including reserved Transfer In), remaining capacity, and whether the post is wholly unoccupied; occupied counts updated by occupancy refresh job", "FR-24, FR-25, FR-48, FR-67, FR-68, FR-78"),
        ],
    )
    ...
    dsr_roles_tbl = insert_table_after(
        dsr_intro,
        ["Division Code (FR-77)", "Role (unique name)", "Description"],
        [
            ("DIV-SECRETARIAT", "ACS / Principal Secretary / Secretary", "Additional Chief Secretary / Principal Secretary / Secretary — hierarchy root"),
            ("DIV-TOP-MGMT", "IGR", "Inspector General of Registration & Commissioner of Stamps"),
            ("DIV-ADMIN", "DIGR (Admin, Law & Computers)", "Deputy IGR for Admin, Law & Computers"),
            ("DIV-ADMIN", "AIGR (Admin)", "Assistant IGR (Admin)"),
            ("DIV-ADMIN", "HQA (Admin)", "Head Quarter Assistant (Admin)"),
            ("DIV-ADMIN", "Sub Registrar (Admin)", "Sub Registrar (Admin) — not to be confused with Sub-Registrar Office (SRO) office type"),
            ("DIV-ADMIN", "Accountant Superintendent (Admin)", "Accountant Superintendent (Admin)"),
            ("DIV-ADMIN", "FDA (Admin)", "First Division Assistant — Admin"),
            ("DIV-ADMIN", "SDA (Admin)", "Second Division Assistant — Admin"),
            ("DIV-ADMIN", "Typist (Admin)", "Typist — Admin"),
            ("DIV-ADMIN", "HQA (RTI)", "Head Quarter Assistant (RTI)"),
            ("DIV-ADMIN", "FDA (RTI)", "First Division Assistant — RTI & Statistics"),
            ("DIV-ADMIN", "SDA (RTI)", "Second Division Assistant — RTI & Statistics"),
            ("DIV-ADMIN", "Statistical Inspector", "Statistical Inspector"),
            ("DIV-VIGILANCE", "DIGR (Vigilance)", "Deputy IGR (Vigilance)"),
            ("DIV-VIGILANCE", "Law Officer", "Departmental Law Officer"),
            ("DIV-COMPUTERS", "AIGR (Computers)", "Assistant IGR (Computers)"),
            ("DIV-COMPUTERS", "System Integrator", "System Integrator / SI support"),
            ("DIV-COMPUTERS", "PMU", "Project Management Unit"),
            ("DIV-COMPUTERS", "Application Developer", "Application development support"),
            ("DIV-COMPUTERS", "HQA / Project Manager (Comp)", "Head Quarter Assistant / Project Manager (Computers)"),
            ("DIV-COMPUTERS", "Sub Registrar (Computers)", "Sub Registrar (Computers)"),
            ("DIV-COMPUTERS", "FDA (Computers)", "First Division Assistant — Computers"),
            ("DIV-COMPUTERS", "SDA (Computers)", "Second Division Assistant — Computers"),
            ("DIV-ENFORCEMENT", "DIGR (Enforcement)", "Deputy IGR (Enforcement)"),
            ("DIV-ENFORCEMENT", "DRO", "District Registrar / DRO"),
            ("DIV-ENFORCEMENT", "HQA (Enforcement)", "Head Quarter Assistant (Enforcement)"),
            ("DIV-ENFORCEMENT", "Sub-Registrar (SR)", "Sub-Registrar (office head / signing)"),
            ("DIV-ENFORCEMENT", "FDA (Enforcement)", "First Division Assistant — Enforcement"),
            ("DIV-ENFORCEMENT", "SDA (Enforcement)", "Second Division Assistant — Enforcement"),
            ("DIV-ENFORCEMENT", "DEO", "Data Entry Operator — SRO operational role"),
            ("DIV-INT-AUDIT", "DIGR (Intelligence)", "Deputy IGR (Intelligence)"),
            ("DIV-INT-AUDIT", "AIGR (Audit)", "Assistant IGR (Audit)"),
            ("DIV-INT-AUDIT", "HQA (Audit)", "Head Quarter Assistant (Audit)"),
            ("DIV-INT-AUDIT", "Superintendent (Audit)", "Superintendent (Audit)"),
            ("DIV-INT-AUDIT", "FDA (Audit)", "First Division Assistant — Audit"),
            ("DIV-INT-AUDIT", "SDA (Audit)", "Second Division Assistant — Audit"),
            ("DIV-INT-AUDIT", "Typist (Audit)", "Typist — Audit"),
            ("DIV-CVC", "DIGR CVC", "Deputy IGR (CVC)"),
            ("DIV-CVC", "JD Town Planning", "Joint Director, Town Planning"),
        ],
    )
    ...
    note6 = insert_paragraph_after(
        ...
        "(FR-51) and maintains all masters in this section (FR-42). Maker-checker / dual authorisation "
        "is not required. DSR and Other Department user creation does not capture security questions.",
        "Normal",
    )
    ...
    hier_seed_tbl = insert_table_after(
        ...
        [
            ...
            ("4", "Computers — Operations", "POST-SR-COMP", "Sub Registrar (Computers)", "POST-HQA-COMP"),
    ...
    hier_attr_tbl = insert_table_after(
        ...
        [
            ...
            ("Division / Branch", "Division Code from Division Master (FR-77)", "Yes"),
    ...
    office_note = insert_paragraph_after(
        ...
        "Hierarchy. MS Building is the Secretariat office (root). Under it is IGR Office (Head Office). "
        "Under the Head Office are District Registrar Offices. Under each District Registrar Office are "
        "Sub-Registrar Offices (FR-59). This "
        ...
        "are actionable (FR-57, FR-60). ACS / Secretary is posted at MS Building (POST-ACS-SEC).",
        "Normal",
    )
    ...
    office_seed_tbl = insert_table_after(
        ...
        [
            ("0", "Secretariat", "OFF-MS-BLDG", "MS Building", "— (root)"),
            ("1", "Head Office", "OFF-IGR", "IGR Office (Head Office)", "OFF-MS-BLDG"),
            ("2", "District Registrar Office", "OFF-DRO-MYS", "DRO Mysuru", "OFF-IGR"),
            ("2", "District Registrar Office", "OFF-DRO-BLR", "DRO Bengaluru", "OFF-IGR"),
            ("3", "Sub-Registrar Office", "OFF-SRO-YESH", "SRO Yeshwanthapura", "OFF-DRO-BLR"),
            ("3", "Sub-Registrar Office", "OFF-SRO-JAY", "SRO Jayanagar", "OFF-DRO-BLR"),
            ("3", "Sub-Registrar Office", "OFF-SRO-MYS-E", "SRO Mysuru East", "OFF-DRO-MYS"),
        ],
    )
    ...
    insert_table_after(
        ...
        [
            (
                "MS Building (OFF-MS-BLDG)",
                "IGR Head Office + DRO and SRO offices under the tree",
                "Only posts that report immediately to ACS (e.g. IGR) — not DIGR or Sub-Registrar",
            ),
            (
    ...
    wf_intro = insert_paragraph_after(
        ...
        "refresh job after midnight IST (FR-68). DSR user creation uses KGID as Username with no "
        "security questions; maker-checker is not required (FR-51). "
    ...
    relieve_note = insert_paragraph_after(
        ...
        "Mapping is removed after 11:59 PM IST of the Relieving Date by the occupancy refresh job "
        "(FR-58, FR-68).",
        "Normal",
    )
    ...
    transfer_in_note = insert_paragraph_after(
        ...
        "to receive a Transfer In. The officer may log in for that post only from 12:00 AM IST on the "
    ...
    transfer_in_ex = insert_paragraph_after(
        ...
        "01-Sep-2026 12:00 AM IST. Shortly after midnight IST on 01-Sep the occupancy refresh job (FR-68) "
    ...
    transfer_in_tbl = insert_table_after(
        ...
        [
            ...
            ("9", "Before 12:00 AM IST of Joining Date — login with that post blocked", "System", "FR-61"),
    ...
    insert_table_after(
        ...
        ["Step", "Action", "Actor", "Notes"],
        [
            ("1", "Start occupancy refresh job shortly after 12:00 AM IST", "System", "FR-68; idempotent"),
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v4.8"
