# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v3.3.docx — multi-post login post selection with office details."""
...
DST = BASE / "BRD_User_Management_v3.3.docx"
...
def build() -> Document:
    ...
    scope_in = {
        ...
        18: "OTP-based authentication (login, logout) — Username + OTP + Captcha; biometrics for departmental users; multi-post DSR login post selection with office details",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "roles (for DSR Officers: from the login-selected post — FR-52); load Role–Module–Function "
                "claims; for each API/URL request look up the Resource Master to obtain the required "
                "Module Function; allow only if the user holds that function; otherwise deny (HTTP 403) "
                "and audit the attempt. Application Admin system principals are authorised for "
            ...
            (
                ...
                "login-selected post via Post–Role mapping (FR-52); for Other Department users, the "
                "single assigned role; for Citizens, assigned Citizen roles.",
                "High",
            ),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Session post", "The single active Post + Office context chosen (or auto-selected) at DSR login; session roles derive from this post via Post–Role mapping"),
    ...
    note2 = insert_paragraph_after(
        ...
        "after authentication before entering the application (FR-52).",
        "Normal",
    )
    ...
    login_post_tbl = insert_table_after(
        ...
        [
            ...
            ("7", "Resolve Module Function claims; enter application", "UM / Auth", "FR-38, FR-41"),
        ],
    )

    sub3 = insert_heading_after(
        login_post_tbl, "6.5.3 Posts Master, Post–Role Mapping, and Sanctioned Posts", "Heading 3", parent=rbac_parent
    )
    note3 = insert_paragraph_after(
        ...
        "post when multiple are assigned (FR-52). Session roles are those mapped to the selected post. "
    ...
    primary_note = insert_paragraph_after(
        ...
        "roles = roles mapped to that selected post via Post–Role mapping. Division-specific posts "
    ...
    primary_tbl = insert_table_after(
        ...
        [
            (
                ...
                "Roles from Post–Role mapping for the login-selected post (FR-52)",
                "Optional per post occupancy",
                "At least one post required; multi-post → choose at login",
            ),
            (
                "Other Department",
    ...
    enf_tbl = insert_table_after(
        ...
        [
            ...
            ("2", "Load session role(s)", "Auth / UM service", "DSR: roles from selected post via Post–Role map; Other Department: one role"),
    ...
    note7 = insert_paragraph_after(
        ...
        "Post has exactly one parent Post in the tree. Application Admin shall add, edit, reorder, "
    ...
    hier_seed_intro = insert_paragraph_after(
        note7,
        "Seed hierarchy (Post → Parent Post) — uses unique Posts Master codes:",
        "Normal",
    )
    hier_seed_tbl = insert_table_after(
        ...
        [
            ...
            ("3", "Enforcement", "POST-SR", "Sub-Registrar", "POST-DIGR-ENF"),
            ("3", "Enforcement", "POST-FDA-ENF", "FDA (Enforcement)", "POST-DIGR-ENF"),
            ("3", "Enforcement", "POST-SDA-ENF", "SDA (Enforcement)", "POST-DIGR-ENF"),
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v3.3"
