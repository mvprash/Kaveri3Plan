# -*- coding: utf-8 -*-
"""Build BRD_User_Management_v4.2.docx — FR-53 revised: login to assigned post only (FR-52); after login, optional
additional charge of a wholly unoccupied subordinate post within the same office without logout or order;
session privileges = union of assigned post + additional charge post roles."""
...
DST = BASE / "BRD_User_Management_v4.2.docx"
...
def build() -> Document:
    ...
    replace_paragraph_text(
        ...
        "capacity at the office (Occupied < Sanctioned Strength), whereas additional charge under a "
        ...
        "without logout or order (FR-53). Other Department users are assigned "
    ...
    scope_in = {
        ...
        18: "Username-based login for all categories (preferred Username for Citizens, KGID for departmental users); OTP to mobile only; Captcha; biometrics for departmental users; Citizen-only lost-mobile reset via three random security questions plus PIN to registered email; FR-52 assigned-post login; FR-53 post-login additional charge; FR-54 display",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "(Office Name and Office Code as available). The user shall select exactly one assigned "
                "post for the session. If the officer has only one active occupancy, the system shall "
                "auto-select that post and skip the selection screen. Login shall complete under that "
                "assigned post only — additional charge of a subordinate post (FR-53) shall not be offered "
                "during the login flow. Initial Module Function claims on entering the home page shall "
                "derive from the assigned post via Post–Role mapping. Other Department and Citizen users "
                "are not subject to post selection.",
                "High",
            ),
            (
                "FR-53",
                ...
                "charge subordinate post at a time. Effective Module Function claims shall be the union of "
                "those mapped to the assigned post and those mapped to the additional charge post (FR-38) — "
                "the officer retains privileges of the assigned post while gaining those of the subordinate "
                "post. The officer may revert to assigned-post-only context at any time during the session "
                "without logout. Selection and reversion shall be audit-logged (actor, assigned post, "
                "additional charge post or cleared, office, timestamp).",
                "High",
            ),
            (
                "FR-54",
                "After login, the home page and/or application header shall display the assigned Post "
                "(from FR-52) with its mapped Role(s) and Office details. When an additional charge "
                "subordinate post is active under FR-53, the header shall also display that post with "
                "its mapped Role(s), clearly labelled as additional charge (e.g. \"Assigned: Sub-Registrar "
                "(SR) — SRO Yeshwanthapura | Additional charge: FDA (Enforcement)\"). The officer may "
                "initiate or clear additional charge from this header or an equivalent in-application "
                "control without logging out.",
                "High",
            ),
            (
                "FR-55",
    ...
    replace_table_rows(
        ...
        [
            ...
            (
                ...
                "roles (for DSR Officers: assigned post from FR-52 plus optional additional charge post "
                "from FR-53); load Role–Module–Function claims per FR-38; for each API/URL request look up the Resource Master "
            ...
            (
                ...
                "mapped to the roles held in that session: for DSR Officers, Module Functions from the "
                "assigned post (FR-52) plus, when an additional charge subordinate post is active under "
                "FR-53, Module Functions from that subordinate post — the officer retains assigned-post "
                "privileges while additional charge is active; for Other Department users, the single "
                "assigned role; for Citizens, assigned Citizen roles.",
                "High",
            ),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Vacancy tests confused in build", "High", "FR-66 defines available capacity and wholly unoccupied separately; assignment / Transfer In use capacity; FR-53 additional charge uses Occupied = 0; union of assigned + additional charge roles per FR-38; both vacancy values on occupancy screens"),
    ...
    replace_table_rows(
        ...
        [
            ...
            ("Wholly unoccupied post", "Vacancy test used only for FR-53 post-login additional charge: the occupied count for a Post at an Office is zero, irrespective of sanctioned strength (FR-66(b))"),
            ...
            ("Additional charge", "Temporary in-office cover of a wholly unoccupied subordinate post under the assigned post, taken after login under FR-53 without logout or order; at most one at a time; privileges = union with assigned post (FR-38)"),
            ...
            ("Session Post–Role display", "After login, persistent display of assigned Post with Role(s); when FR-53 is active, also shows additional charge Post with Role(s) (FR-54)"),
    ...
    switch_note = insert_paragraph_after(
        ...
        "nobody is posted against it at that Office (Occupied = 0 — FR-66(b)). The officer keeps "
        "privileges of the assigned post and gains those of the additional charge post (FR-38). At most "
        "one additional charge post may be active at a time; the officer may clear it and return to "
        "assigned-post-only context without logging out.",
        "Normal",
    )
    switch_tbl = insert_table_after(
        ...
        [
            ...
            ("6", "Officer selects subordinate post or clears additional charge", "DSR Officer", "No order required"),
            ("7", "Recompute Module Function claims — union of assigned + additional charge (FR-38)", "UM / Auth", "Assigned-post privileges retained"),
            ("8", "Update header — assigned post + optional additional charge label (FR-54)", "UI", "Both contexts visible"),
    ...
    switch_ex_tbl = insert_table_after(
        ...
        [
            (
                ...
                "Assigned: Sub-Registrar (SR) — SRO Yeshwanthapura (FR-54)",
            ),
            (
                "SRO Yeshwanthapura",
                "→ FDA (Enforcement)",
            ...
            (
                ...
                "—",
                "—",
                "SR may take additional charge as DEO; retains SR signing rights (FR-38)",
                "Header: Assigned SR + Additional charge DEO (FR-54); clear without logout",
            ),
        ],
    )
    ...
    display_note = insert_paragraph_after(
        ...
        "Office, and Role(s) from login (FR-52). When additional charge is active, the header shall also "
        "show the subordinate Post and Role(s) with an \"Additional charge\" label. Example without "
        "additional charge: \"Assigned: Sub-Registrar (SR) | SRO Yeshwanthapura\". Example with "
        "additional charge: \"Assigned: Sub-Registrar (SR) | SRO Yeshwanthapura | Additional charge: "
        "FDA (Enforcement)\". The officer opens the additional-charge control from here without logging out.",
        "Normal",
    )
    ...
    note3 = insert_paragraph_after(
        ...
        "selects one active post when multiple are assigned (FR-52). Effective access is the union of "
        "assigned-post roles and, when active, additional-charge subordinate roles (FR-38, FR-53). "
        "Over-capacity assignment is blocked (FR-26, FR-27, FR-45). FR-66: assignment and Transfer In "
        "need available capacity (Occupied < Sanctioned Strength); FR-53 additional charge needs a wholly "
    ...
    primary_tbl = insert_table_after(
        ...
        [
            (
                ...
                "Roles from assigned post (FR-52) plus optional additional charge (FR-53); union per FR-38",
                "Optional per post occupancy",
                "Multi-post → choose assigned post at login; additional charge after login without logout",
            ),
            (
                "Other Department",
    ...
    enf_tbl = insert_table_after(
        ...
        [
            ...
            ("2", "Load session role(s) — assigned post plus optional additional charge", "Auth / UM service", "Union per FR-38"),
    ...
    core.title = "BRD — User Management Module (KAVERI 3.0) v4.2"
