...
| **Programme constraint** | **All modules Go Live within 11 months**; **Phase 1 Go Live by 3rd week of October 2026**; **Kaveri 1.0/2.0 data migration in-scope** |
...
| 0.4 | 2026-08-20 | Delivery | Added **Data Migration workstream** from Kaveri 1.0 / 2.0 ecosystem (phased cutovers, validation, dual-run); aligned to Phase 1–4 Go Lives |
...
| MIG-K3-PROG-001 | Data Migration strategy & phase exit reports (to be maintained) |
...
9. Go Live  
10. **Data Migration** from Kaveri 1.0 / 2.0 ecosystem  
...
| O6 | Migrate required operational & historical data from Kaveri 1.0/2.0 without service break beyond approved freeze windows | Migration exit reports + reconciliation sign-off per phase |
...
**Data Migration (new in v0.4):** Continuous workstream from M1 through each phase cutover — see **§5A**.
...
**In scope:** Structured/operational data migration from Kaveri 1.0 & 2.0 (databases, registers indexes, users, masters, EC indexes, marriage history as agreed).  
**Deferred** unless Steering promotes: #32 paper/legacy **image digitisation** (scan backlog beyond existing digital records), #34 Accounts, #48 Mobile, #49 Training module product (training still executed as change stream).
...
| DBA + Migration | 1+1 = 2 | Continuous; peaks Phase 1 cutover + Phase 3 EC/registers |
...
| DBA | 1.0 | 0.5 | 1.0 | 0.3 |
| Data Migration | 1.0 | 0.8 | 1.0 | 0.6 |
...
| **Data Migration (Must)** | DM-P1-01..06 — masters, users, marriage history/in-flight, images, fee refs | Freeze+delta **15–20 Oct**; K3 SoT from **21-10-2026** |

Marriage (Online eSign + Offline DEO), Certified Copies, User Management, eKYC, eSign, Khajane payment, Scanning spine, Marriage Dashboard & MIS; Refund/Audit/MDM foundation at GL with completion in P1.1; **Kaveri 1.0/2.0 data cutover for Phase 1 domains**.
...
**Data Migration:** DM-P2-01..04 rate cards, guideline/CVC tables, GIS valuation keys from Kaveri 2.0 (and 1.0 where needed) before **16-03-2027** GL.
...
**Data Migration (largest package):** DM-P3-01..07 registration books/transactions, pendency, filings, **EC indexes from 1.0+2.0**, open DRO/IGRO cases, deed image re-link — continuous Oct 2026 → cutover **08-06-2027**.

### 4.4 Phase 4 — Firm Registration

Firm Registration (DRO #41), Firm MIS/Dashboard/Audit; reuse Phase 1 platform.  
**Data Migration:** DM-P4-01..02 firm masters, certificates, open amendments from Kaveri 1.0/2.0 before **14-07-2027** GL.
...
## 5A. Data Migration — Kaveri 1.0 / 2.0 ecosystem

### 5A.1 Purpose

Migrate **operational, master, transactional, and searchable historical data** required for Kaveri 3.0 phase go-lives from the current **Kaveri 1.0 and Kaveri 2.0** ecosystem (applications, databases, file/object stores, and approved peripheral systems) so that citizens and officers can continue work on day-1 of each phase without depending on the legacy UI for in-scope modules.

This workstream is owned by the **Data Migration Specialist** with DBA, Domain Expert, Tech Leads, Security, and BA support (K3_ITCellRequirement.pdf Sl.9).

### 5A.2 In scope vs out of scope

| In scope | Out of scope (unless Steering promotes) |
|----------|----------------------------------------|
| Structured DB data from Kaveri 1.0 / 2.0 for modules going live in each phase | Fresh **paper** record digitisation / bulk scan backlog (#32) beyond already-digital images |
| User / role / office mappings needed for User Management | Rebuild of unrelated departmental systems (Bhoomi, etc.) except agreed integration keys |
| Masters (district, SRO, holidays, fee schedules, reason codes) | Accounts module full ledger migration (#34) |
| Marriage applications / registers / certificates (as digitally available) | Mobile-app-only datasets (#48) |
| Document registration books/indexes, appointments, pendency, Rule 17/filing metadata | Redesign of source-of-truth outside Registration dept |
| EC search indexes and linked document references | Changing historical legal facts (correction via 68(2) / business process, not silent ETL rewrite) |
| Stamp / guideline / CVC published value tables | |
| Firm registration records for Phase 4 | |
| Already-stored scanned images / PDFs referenced by legacy IDs (re-key / re-link) | |
| Audit trail **minimum** required for continuity (as decided with Legal/Security) | Full verbatim multi-year audit dump if volume blocks cutover — archive strategy instead |

### 5A.3 Source landscape (to be inventoried in M1)

| Source class | Examples (indicative) | Consumers in K3 |
|--------------|----------------------|-----------------|
| Kaveri 1.0 core DB(s) | Older registration / marriage / EC schemas | Phase 1–3 historical depth |
| Kaveri 2.0 core DB(s) | Current operational schemas, workflow states | Phase 1–4 cutover (primary) |
| Document / image stores | Scanned deed pages, marriage docs, CC artefacts | Object store re-link |
| Identity / LDAP / local user stores | Officer logins, office mapping | User Management |
| Payment / Khajane references | Receipt IDs, recon keys | Payment + MIS recon |
| Valuation / guideline pubs | CVC / guideline tables, GIS keys | Phase 2 calculators |
| Firm / DRO stores | Firm register data | Phase 4 |
| Reports / MIS extracts | Where DB incomplete | BI seed (secondary) |

**M1 deliverable:** *Source Inventory & Access Pack* (schemas, row counts, PII classes, retention, owners, connection method).

### 5A.4 Migration principles

1. **Phase-aligned cutovers** — migrate only what that phase needs to go live; avoid big-bang statewide dump.  
2. **Kaveri 2.0 is primary cutover source**; Kaveri 1.0 supplies historical depth where 2.0 is incomplete.  
3. **Idempotent ETL** — re-runnable pipelines with watermark / batch IDs.  
4. **No silent legal mutation** — transform for schema only; business corrections via module workflows.  
5. **Reconcile before cutover** — count, hash, sample, and domain spot-checks signed by Domain Expert.  
6. **Dual-run / freeze window** — short write-freeze on legacy module; delta catch-up; then K3 becomes SoT for that module.  
7. **PII & Aadhaar-safe** — mask in non-prod; production migration under Security Specialist controls.  
8. **Rollback pack** — legacy remains read-only fallback for agreed hypercare window per phase.

### 5A.5 Approach (per phase)

`	ext
Discover → Map (1.0/2.0 → K3) → Cleanse rules → Build ETL → Dry-run (non-prod)
    → Reconcile → UAT with migrated data → Freeze + Delta → Cutover → Hypercare audit
`

| Stage | Artefact | Gate |
|-------|----------|------|
| Discover | Source inventory, volume & quality report | Access granted; counts known |
| Map | Field mapping workbook + transformation rules | Architect + Domain sign-off |
| Build | ETL jobs, error quarantine, re-run playbooks | Code review + Security |
| Dry-run | Non-prod full/partial load | > agreed accuracy threshold |
| Reconcile | Count/hash/sample report | Domain Expert sign-off |
| Cutover | Freeze notice, delta load, SoT switch | PO + PM + Domain |
| Close | Migration exit report + archive plan | Steering (per phase) |

### 5A.6 Data domains by delivery phase

| Phase | Go Live | Migration packages (Must for GL) | Should / follow-up |
|-------|---------|----------------------------------|--------------------|
| **Phase 1** | **21-10-2026** | **DM-P1-01** Office/SRO/district masters; **DM-P1-02** Users, roles, transfer/relieving history (agreed depth); **DM-P1-03** Marriage applications in-flight + completed registers/certificates (digital); **DM-P1-04** Marriage fee/receipt references; **DM-P1-05** Scan/image pointers for marriage/CC; **DM-P1-06** Holiday / fee stub masters | Full multi-year marriage archive beyond agreed depth; Refund/Audit/MDM enrich in P1.1 |
| **Phase 2** | **16-03-2027** | **DM-P2-01** Stamp Act schedule / fee rate cards from 2.0; **DM-P2-02** Guideline / market value tables; **DM-P2-03** CVC publication sets; **DM-P2-04** GIS parcel keys / valuation refs (as licensed) | Historical rate cards older than policy window (archive) |
| **Phase 3** | **08-06-2027** | **DM-P3-01** Document registration transactions + book/volume/serial; **DM-P3-02** Appointment & pendency; **DM-P3-03** Filings / Rule 17 / memo metadata; **DM-P3-04** EC indexes (1.0+2.0) + document links; **DM-P3-05** PoA / verify / court / liability open items; **DM-P3-06** DRO undervaluation / will / adjudication open cases; **DM-P3-07** Deed scan re-link at scale | Deep historical EC beyond SLA years (nearline archive); IGRO closed appeals archive |
| **Phase 4** | **14-07-2027** | **DM-P4-01** Firm registration master & certificates; **DM-P4-02** Open amendments / renewals | Dormant firms beyond retention (archive) |

### 5A.7 Migration timeline (aligned to programme)

| Window | Activity | Owner |
|--------|----------|-------|
| 17-08 → 05-09-2026 | Source inventory (1.0 + 2.0); access; PII classification; volume baselines | Migration + DBA + Security |
| 01-09 → 20-09-2026 | Phase 1 mapping (users, masters, marriage, images) | Migration + BA + Domain |
| 15-09 → 05-10-2026 | Phase 1 ETL build + dry-runs in SIT | Migration + DBA + DevOps |
| 06-10 → 14-10-2026 | Phase 1 UAT **on migrated data**; reconcile sign-off | QA + Domain + Migration |
| **15-10 → 20-10-2026** | **Phase 1 freeze + delta + cutover** with app deploy | PM + Migration + DevOps |
| **21-10-2026** | Phase 1 Go Live with K3 as SoT for P1 modules | Steering |
| 21-10 → 15-11-2026 | P1.1 delta fixes; Refund/Audit/MDM data hooks | Migration |
| Nov 2026 → Jan 2027 | Phase 2 mapping/ETL/dry-run (rates, guideline, CVC, GIS) | Migration + Domain |
| Feb → mid-Mar 2027 | Phase 2 reconcile + cutover with P2 GL **16-03-2027** | Migration |
| Oct 2026 → Apr 2027 | Phase 3 continuous: regn + EC index build (largest volume) | Migration + DBA |
| Mar → May 2027 | Phase 3 full dry-run, EC accuracy pilots, cutover rehearsals | Migration + QA |
| Late May → **08-06-2027** | Phase 3 freeze/delta/cutover with Document + EC GL | Migration + PM |
| Apr → Jul 2027 | Phase 4 firm data map/ETL/cutover with **14-07-2027** GL | Migration |

### 5A.8 Environments & tooling

| Item | Approach |
|------|----------|
| Non-prod | Masked subset + periodic full clone for dry-run (Security-approved) |
| Prod migration bus | Controlled ETL window; Kafka/files as needed; DBA supervised loads |
| Quarantine | Reject/exception tables with repair workflow (no drop of source rows) |
| Observability | Row counts in/out, error rate, duration dashboards (BI optional) |
| Security | Encryption in transit; access break-glass; Aadhaar fields per UIDAI norms |

### 5A.9 Validation & acceptance (every phase)

| Check | Pass criteria (indicative — freeze in DEC log) |
|-------|-----------------------------------------------|
| Record counts | Source vs target within agreed tolerance (e.g. 0 open Must gaps) |
| Financial keys | Receipt / fee totals match for sample period |
| Identity | 100% active officers mapped or explicitly disabled with reason |
| Marriage / Regn / Firm | Spot-check certificates & register numbers by Domain Expert |
| EC | Pilot SRO search parity vs 2.0 for agreed query pack |
| Images | Spot-check open+view for linked docs |
| Performance | EC / register queries meet NFR on migrated volume |
| Sign-off | Migration Exit Report signed: Migration Specialist, DBA, Domain, PO, Security |

### 5A.10 Dual-run, freeze, and rollback

| Phase | Legacy posture after GL | Rollback |
|-------|-------------------------|----------|
| 1 | Kaveri 2.0 Marriage/User paths read-only or disabled for migrated offices; 1.0 archive online if needed | Re-enable 2.0 read/write within hypercare if Sev-1 data defect |
| 2 | Calculators SoT in K3; legacy rate screens deprecated | Fall back to published PDF/guideline circulars + legacy calc if needed |
| 3 | Document + EC SoT in K3 post cutover; legacy EC read-only during hypercare | Pilot office rollback plan rehearsed before statewide |
| 4 | Firm SoT in K3 | DRO legacy firm module rollback window 2 weeks |

### 5A.11 Team & RACI (migration)

| Activity | Migration Spec. | DBA | Domain | BA | Architect | Security | PO/PM |
|----------|:---:|:---:|:---:|:---:|:---:|:---:|:---:|
| Inventory & access | A/R | R | C | C | C | C | I |
| Mapping rules | R | C | A | R | C | C | C |
| ETL build | A/R | R | C | I | C | C | I |
| Reconcile sign-off | R | R | A | C | I | C | A (PO) |
| Cutover command | R | R | C | I | C | C | A (PM) |
...
7. **Migrate with each phase** — no phase Go Live without agreed Kaveri 1.0/2.0 data cutover for that phase’s modules.  
8. **Scope discipline** — Mobile, Accounts, paper digitisation backlog stay out unless Steering swaps scope.
...
|------:|----------|--------|
| M1 | Aug 2026 | BR Marriage/User Mgmt; Phase 1 HLD + **build starts 25-08**; platform AD; **start K1/K2 source inventory** |
| M2 | Sep 2026 | Document BR; Phase 1 build; adapters SIT; Phase 2 design; **P1 ETL map/build + dry-run** |
| M3 | Oct 2026 | CC MVP; **P1 migrate freeze/delta + Go Live 21-10**; BR ends 15-10; P1.1; Phase 2/3 ramp; **start P3 EC index pipeline** |
...
| Cutover | 15-10 → 21-10 | Pre-Prod soak, Prod deploy, **data freeze+delta**, smoke, **Go Live 21-10** | DevOps + Migration + all |
| DM-P1 | 01-09 → 20-10 | Inventory → map → ETL → dry-run → UAT-on-data → cutover | Migration + DBA |
...
- [ ] **DM-P1 migration exit report signed** (counts, users, marriage sample, images)
- [ ] Legacy Marriage/User write path frozen per cutover plan
...
| D3 | Feb–Apr | EC + **Kaveri 1.0/2.0 EC index migration** cutover rehearsal |
...
- [ ] **Data Migration Exit Report** for that phase (Kaveri 1.0/2.0 → K3)  
- [ ] Legacy freeze / dual-run / rollback pack confirmed  
- [ ] Security + perf + DR evidence  
- [ ] Runbooks + L2 roster  
- [ ] Training (Content + Transition)  
- [ ] Rollback criteria  
- [ ] PO + PM + Architect + Domain + Migration Specialist + Steering sign-off  
...
| S6 | Phase data migrations accepted (DM-P1..P4 exit reports) | Per phase GL dates |
...
| M-DM-INV | Kaveri 1.0/2.0 source inventory complete | 05-09-2026 | Migration / DBA |
| M-DM-P1 | Phase 1 migration exit signed | 20-10-2026 | Domain / PO |
| M-P1-GL | Phase 1 Go Live (3rd week Oct) | **21-10-2026** | Steering |
| M-DM-P2 | Phase 2 migration exit signed | 13-03-2027 | Domain / PO |
| M-P2-GL | Phase 2 Go Live | **16-03-2027** | Steering |
| M-DM-P3 | Phase 3 migration exit signed (incl. EC) | 05-06-2027 | Domain / PO |
| M-P3-GL | Phase 3 Go Live | **08-06-2027** | Steering |
| M-DM-P4 | Phase 4 migration exit signed | 11-07-2027 | Domain / PO |
...
| Kaveri 1.0/2.0 DB + image store **read access** | M1 (by 05-09-2026) | All migrations slip; P1 GL at risk |
| Phase 1 migration dry-run complete | 05-10-2026 | P1 UAT-on-data blocked |
| EC historical index migration (1.0+2.0) | Phase 3 cutover | EC not statewide by 08-06-2027 |
| Act 47 SOPs | Phase 3 SDD | Refusal/cancel gaps |
| Firm Act set freeze (Sr.29–31) | Phase 4 design | Rework in M9–M11 |
| Legacy write-freeze authority (circular) | Each phase cutover | Dual SoT / data divergence |
...
| R-00b | Phase 1 data migration incomplete by 21-10 | Cannot cut over Marriage/Users | Parallel DM-P1 from 01-09; UAT only on migrated data; narrow historical depth if needed | Migration / PO |
...
| R-07 | EC / register migration quality or volume | Phase 3 GL incomplete | Start EC pipeline Oct 2026; pilot SRO parity; nearline archive for deep history | Migration / DBA |
| R-07b | Kaveri 1.0 schema poorly documented / dual sources diverge | Mapping delays | Inventory M1; 2.0 as primary; 1.0 as historical fill with explicit rules | Migration / Architect |
| R-07c | Image/object re-link failures at scale | CC/deed view broken | Early sample migrate; checksum; quarantine + repair queue | Migration / DevOps |
| R-08 | Scope creep (mobile/paper digitisation/accounts) | Breaks 11-month | Change board; swap-in only with equal swap-out | Steering |
...
7. Start Stamp Schedule 2022 digitisation after Sr.13–14.  
8. **Stand up Data Migration workstream immediately:** Kaveri 1.0/2.0 source inventory, access, PII classification (complete by **05-09-2026**).  
9. Freeze Phase 1 field-mapping workbook (users, masters, marriage, images) by **20-09-2026**.  
10. Schedule Phase 1 **legacy write-freeze** window (**15–20 Oct 2026**) with department circular.  
...
| Kaveri 1.0/2.0 data packages DM-P1..P4 | All (with each phase) | With each phase GL |

## Appendix C — Changes across versions

| Item | v0.1 | v0.2 | v0.3 | v0.4 |
|------|------|------|------|------|
| Phase 1 GL | 23-03-2027 | 02-02-2027 | **21-10-2026** | **21-10-2026** |
| Phase 2 GL | 12-07-2027 | 13-04-2027 | 16-03-2027 | 16-03-2027 |
| Phase 3 GL | 24-01-2028 | 30-06-2027 | 08-06-2027 | 08-06-2027 |
| Phase 4 GL | 11-09-2028 | 14-07-2027 | 14-07-2027 | 14-07-2027 |
| Data migration K1/K2 | Not planned | Mention only (EC) | Mention only | **Full workstream §5A + DM-P1..P4** |
| Staffing | Indicative | IT Cell 39 | IT Cell 39 | IT Cell 39; Migration Spec. continuous |
| Phase 1 mode | Staged | Compressed | ~9-week sprint | ~9-week sprint **+ mandatory data cutover** |

## Appendix D — Migration package ID quick list

| ID | Phase | Package |
|----|------:|---------|
| DM-P1-01 | 1 | Office / district / SRO masters |
| DM-P1-02 | 1 | Users, roles, transfer/relieving |
| DM-P1-03 | 1 | Marriage in-flight + digital registers/certificates |
| DM-P1-04 | 1 | Marriage fee / receipt references |
| DM-P1-05 | 1 | Marriage/CC image pointers |
| DM-P1-06 | 1 | Holiday / fee stub masters |
| DM-P2-01 | 2 | Stamp schedule / rate cards |
| DM-P2-02 | 2 | Guideline / market value tables |
| DM-P2-03 | 2 | CVC publication sets |
| DM-P2-04 | 2 | GIS valuation keys/refs |
| DM-P3-01 | 3 | Document registration + book/volume/serial |
| DM-P3-02 | 3 | Appointment & pendency |
| DM-P3-03 | 3 | Filings / Rule 17 / memo metadata |
| DM-P3-04 | 3 | EC indexes (1.0+2.0) + links |
| DM-P3-05 | 3 | PoA / verify / court / liability open items |
| DM-P3-06 | 3 | DRO open cases (UV / will / adjudication) |
| DM-P3-07 | 3 | Deed scan re-link |
| DM-P4-01 | 4 | Firm masters & certificates |
| DM-P4-02 | 4 | Firm open amendments / renewals |

---

*End of Programme Project Plan v0.4 — Phase 1 Go Live 21-10-2026; Kaveri 1.0/2.0 data migration workstream; 11-month all-modules close; resources per K3_ITCellRequirement.pdf.*
